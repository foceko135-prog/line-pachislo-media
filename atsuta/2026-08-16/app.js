...
function nrIntro(D){
  ...
  return '<figure class="nr-fig">'
    +'<img src="'+esc(asrc("nerai_yomikata.jpg"))+'" alt="マスの読み方"'
    +' loading="lazy" decoding="async">'
    +'<figcaption>マスの読み方（押すと大きく見られます）</figcaption></figure>'
    +'<ul class="nr-intro">'
    +'<li>各マスは「その狙い方で選んだ台を翌日打ったときの実績」。選ぶときに使うのは'
    +'前日までのデータだけで、当日の結果は見ていません。</li>'
    // ★「上段が出率、下段が平均差枚と件数」は上の図が言っているので消した
    //   (2026-08-16・谷川氏指示。memory: figure-replaces-text-rule)。
    //   色の意味は図に入っていないので残す。
    +'<li>色は下に出ている全体平均が中心で、良いほど緑・悪いほど赤。'
    +'件数の少ないマスは色を薄くしています。</li>'
...
function aiHeadHtml(idx,base){
  ...
  if(aiMode==="ai"){
    return head
      ...
      +' loading="lazy" decoding="async">'
      +'<figcaption>行の読み方（押すと大きく見られます）</figcaption></figure>'
      ...
      +'（重み '+AI_W.map(c=>c.n+Math.round(c.w*100)).join("・")+'）。'
      +'100%が損得なし。<br>'
      // ★確度(2026-08-14夜・谷川氏指示「当てにしてよい予想の%も機種毎、台番毎に」)。
      //   定義は上の図にあるので、ここは**目安の数値だけ**にする。
...
(()=>{
  ...
  body.addEventListener("click",e=>{
    ...
    if(fg){
      e.stopPropagation();
      if(typeof openPhoto==="function") openPhoto(fg.src,"行の読み方");
      return;
    }
...
(function () {
  ...
  body.addEventListener("click", e => {
    ...
    if (fig) {
      e.stopPropagation();
      if (typeof openPhoto === "function") openPhoto(fig.src, "マスの読み方");
      return;
    }
    // 「どれを見るか」のタブ
