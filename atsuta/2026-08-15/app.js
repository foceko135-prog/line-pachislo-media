...
function renderDay(dai){
  ...
  document.getElementById("mcap").textContent=noGraph
  ...
  document.getElementById("chart").innerHTML=noGraph
...
function fitTabbar(){
  ...
  for(const id of ["zoomOut","mvClose","ftClose"]){
    ...
    el.style.right="auto";
    el.style.left=(vv.offsetLeft+vv.width*vv.scale-el.offsetWidth-12)+"px";
...
function nrIntro(D){
  if(!D) return "";
  return '<ul class="nr-intro">'
    +'<li>各マスは「その狙い方で選んだ台を翌日打ったときの実績」。選ぶときに使うのは'
    +'前日までのデータだけで、当日の結果は見ていません。</li>'
    +'<li>上段が出率（機械割）、下段が平均差枚と件数。色は下に出ている全体平均が'
    +'中心で、良いほど緑・悪いほど赤。件数の少ないマスは色を薄くしています。</li>'
    +'<li>「期待設定」は、その台のBIG・REG・ブドウの出方から'
    +'「設定いくつだったと考えるのが自然か」を数字にしたもの'
    +'（台番カードの内容詳細に出るものと同じ計算）。'
    +'ボーナス回数の記録がある機種だけに出ます。</li>'
    +'<li>紺の太枠は損得なしの線 '+esc(String(D.be))+'% 以上のマス'
    +'（46枚貸し・5.06枚交換・持ち玉90%で計算。100%ではありません）。</li>'
...
function buildDocs(){
  ...
  if(TK.days&&TK.days.length){
    ...
    html+=sec("toku","特定日（キャラ誕・記念日）",
      '<table class="dtoku">'+COLS+'<tbody>'+rows+'</tbody></table>'+cyc+hl,false,
  ...
  if(TR.raiten||TR.torizai||TR.keihin||TR.osusume){
    ...
    if(TR.raiten&&TR.raiten.length){
      ...
      TR.raiten.forEach(e=>{
        ...
        if(kishu) head+='<span class="tz-tag tz-k">機種 '+esc(kishu)+'</span>';
        ...
        const nt=e.n?'<div class="tz-n">'+esc(e.n)+'</div>':"";
        r+=drow(e.d,head,perf+nt);
      });
      html+=sec("raiten","来店",wrap(r),false,
        {pre:["新しい順。演者名の右が推しメーカー系と推し機種",
...
const FNUM=["fVmin","fVmax","fRmin","fRmax","fNmin","fNmax","fGmin","fGmax",
            ...
            // 設定の推定(2026-08-13)。RB・ブドウそれぞれの推定設定の範囲
            "fBmin","fBmax","fKmin","fKmax"];
...
function setteiOf(dai,per){
  ...
  return {kitai:kitai, rb:hNearF(o.rbD,sRB), ko:hNearF(kod,sKO), lv:lv};
}
...
["fSue","fPos","fDow","fToku","fSet"].forEach(id=>{
...
function ensureHiruData(){
  ...
  fHiruReq=fetch("hiru.json",{cache:"no-store"}).then(r=>r.ok?r.json():null).then(j=>{
    ...
    if(!j||!j.v||j.date!==ymd) return null;
...
document.querySelectorAll(".fpre .pre").forEach(b=>{
  b.addEventListener("click",()=>{
    ...
    // 期間も既定へ戻してから当てる(2026-08-11)。前に選んだ期間が
    // 残っていると、同じボタンを押しても違う結果になってしまう。
    setFPer(PER_DEF.fPer);
    // 指定は data-set="fRmin=105,fVmin=1" 形式。**data-fRmin のような書き方は使えない**
    // (HTMLのdata属性は小文字化されるので、大文字を含むid(fRmin)と結び付かない)。
    // 期間だけは入力欄ではないので data-set="fPer=last7" の形で受ける
    // (2026-08-12・それまでは fVper= と fRper= の2つ)。
    (b.dataset.set||"").split(",").forEach(kv=>{
      const [k,v]=kv.split("=");
      const key=(k||"").trim();
      if(key==="fPer"){ setPer(key,(v||"").trim()); return; }
...
document.getElementById("fApply").addEventListener("click",()=>{
  ...
  const fBmin=parseNum("fBmin"), fBmax=parseNum("fBmax");
  const fKmin=parseNum("fKmin"), fKmax=parseNum("fKmax");
  const useSet=setc.length>0||fBmin!=null||fBmax!=null||fKmin!=null||fKmax!=null;
...
// チップ(期間・末尾・位置区分・曜日・特定日・設定)とカレンダーの日付。
// 各ボックスの既存ハンドラが状態を切り替えた**あと**に動くよう、親側のバブリングで受ける。
// プリセット(.fpre .pre)は自前で #fApply を押すのでここでは拾わない(二重発火の回避)。
(function(){
...
function paintFilterList(){
  ...
  names.forEach(nm=>{
    const rows=g[nm].sort((a,b)=>(b.v||-1e9)-(a.v||-1e9));
    ...
    h+='<div class="fl-g"><div class="fl-gn">'+esc(nm)
      +'<span>'+rows.length+'台'
...
function repIntent(cur){
  ...
  if(cur.set.length||N.fBmin!=null||N.fKmin!=null) a.push("設定が入っていそうなノーマル機");
...
(()=>{
  ...
  const open=n=>{
    ...
    const future=(dayDiff(n.date)||0)>0;
    ...
    document.getElementById("noticeCard").innerHTML=
      ...
      +esc(n.note||"")+"</div>"
      +(future
        ? ((plan?('<div class="nsec2">この日に入る予定（ホール告知）</div>'+plan):"")
           +'<div id="iretae"></div>')
    ...
    fetch("iretae.json",{cache:"no-store"}).then(r=>r.ok?r.json():null).then(ir=>{
      ...
      const irHead=future
        ? ('<div class="nsec2 is-past">前回 '
           +esc(ir.date?mdw(ir.date):"")+' の入替の内訳'
           +'<span>（実際に入れ替わった台。上の予定とは別のものです）</span></div>')
        : "";
      host.innerHTML=irHead+ir.cats.map(c=>{
...
function aiCalHtml(oneLine){
  ...
  return '<div class="ai-cal"><b>予想の数値は順位を付けるためのものです。</b>'
    ...
    +'<span class="ai-cal-s">過去56日で確かめると、予想が損得なしの線'
    +'（<b>'+AI_BE.toFixed(1)+'%</b>）を超えた11日のうち、'
    +'実際に超えて終われたのは1日だけでした。'
...
function nrTapNote(){
  return '<div class="nr-intro" style="padding-left:0">'
    +'各マスのいちばん下の<b>「◯台」</b>は、<b>いまその狙い方に当てはまる台の数</b>です'
    +'（その上の「◯件」は過去に何回そうなったかの母数）。'
    +'マスを押すと、その台の一覧が出て島図を光らせられます。</div>';
}
...
function aiHeadHtml(idx,base){
  ...
  if(aiMode==="ai"){
    return head
      ...
      +'地力（全期間）・曜日（'+t.wd+'曜）・日付（'+esc(aiDgtName())+'）・'
      +'直近'+AI_REC_DAYS+'日 の4つを、母数の薄いものほど弱く効かせて足しています'
      +'（重み '+AI_W.map(c=>c.n+Math.round(c.w*100)).join("・")+'）。<br>'
      +'100%が損得なし。各行の下に4つの内訳を出しているので、'
      +'どれが効いて上位に来たのかを確かめてから決めてください。<br>'
      +'出率中央＝全期間の実績（見立ての土台）。<br>'
      // ★確度(2026-08-14夜・谷川氏指示「当てにしてよい予想の%も機種毎、台番毎に」)。
      //   意味を必ず添える。数字だけ出すと「勝てる確率」と読まれる。
      +'<b>確度</b>＝その'+(aiNm?"台":"機種")+'について4つの材料がどれだけ厚く'
      +'揃っているかの度合い（100%が満点）。'
      +'<span class="ai-cau">当たる確率ではありません。</span>'
      +'低いほど「地力（これまでの実績）そのまま」に近づきます。'
      +'目安は 機種で60〜90%、台番で25〜50%。'
      +AI_CONF_THIN+'%未満は色を変えています。<br>'
      // ★歩進検証(ai_rank_backtest.py・過去56日)で実際に測った差。
      //   数字を出すだけでは「で、どうすればいいのか」が分からないので、
      //   **測った結果そのもの**を書いて選び方につなげる(2026-08-14夜)。
      +'過去56日で確かめると、台番は<b>確度の高い側から選ぶとプラスで終わる割合が'
      +'全体より+9.1pt、低い側から選ぶと−4.0pt</b>でした'
      +'（低い側は全体平均より悪い＝選ばない方がよい）。'
...
function aiPaintMachines(){
  ...
  if(Object.keys(KTYPE).length){
    ...
    tbl+='<div class="ai-cols">'
      +'<div class="ai-col">'
      +aiMachineHtml(idx,base,"AT機 TOP10",(d,m)=>aiKind(m.n||"")==="at",oAt)+'</div>'
      +'<div class="ai-col">'
      +aiMachineHtml(idx,base,"ノーマル機 TOP10",(d,m)=>aiKind(m.n||"")==="nm",oNm)
      +'</div></div>';
    if(unkN){
      tbl+='<div class="ai-note">種別が取れていない'+unkN+'機種は、どちらの表にも'
        +'入れていません（分からないものを混ぜないため）。</div>';
    }
  }else{
  ...
  h+='<div class="ai-note">機種名を押すと、その機種の中の台番TOP10が出ます。<br>'
    ...
    +'並びの既定は出率の中央値の高い順（同じときは母数の多い順）。<br>'
...
function aiPaint(){
  if(aiNm) aiPaintDais(aiNm); else aiPaintMachines();
}
...
(()=>{
  ...
  if(back) back.addEventListener("click",()=>{ aiNm=null; aiPaint(); });
  ...
  body.addEventListener("click",e=>{
    ...
    if(go){ aiNm=String(go.dataset.nm||""); aiPaint(); return; }
