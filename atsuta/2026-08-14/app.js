...
function fitAiModal(){
  const c=document.getElementById("aiCard");
...
function aiSortRows(rows){
  const k=aiSortK, d=aiSortD;
  const val=x=>{
    const v=x[k];
    return (typeof v==="number")?v:null;
  };
...
function aiMachineHtml(idx,base,ttl,pick){
  ...
  h+='<table class="ai-t ai-t6">'
    ...
    +rows.map((x,i)=>{
      ...
      return '<tr><td class="ai-k"><span class="ai-rk">'+(i+1)+'</span>'
        ...
        +(pred?('<em>内訳: '+esc(x.why||"")+'</em>')
              :('<em>高出率 '+x.hiN+' / '+x.n+'台日'
                ...
                +' ／ プラス '+aiPct(x.pl)+'</em>'))
        // どの旗でこちら側に入れたのかを1行で出す(2026-08-14)。
        // 分類は画面の見出しを決めるものなので、根拠が見えないと直しようがない。
        +(aiKindWhy(x.k)?('<em>種別: '+esc(aiKindWhy(x.k))+'</em>'):"")
        ...
        +'台番詳細 ▸</button></td>'
        +(pred?('<td class="'+aiCl(x.pred==null?null:x.pred-100)+'">'+aiPct(x.pred)+'</td>')
...
function aiPaintMachines(){
  ...
  if(Object.keys(KTYPE).length){
    ...
    h+='<div class="ai-cols">'
      +'<div class="ai-col">'
      +aiMachineHtml(idx,base,"AT機 TOP10",(d,m)=>aiKind(m.n||"")==="at")+'</div>'
      +'<div class="ai-col">'
      +aiMachineHtml(idx,base,"ノーマル機 TOP10",(d,m)=>aiKind(m.n||"")==="nm")+'</div>'
      +'</div>';
    if(unkN){
      h+='<div class="ai-note">種別が取れていない'+unkN+'機種は、どちらの表にも'
        +'入れていません（分からないものを混ぜないため）。</div>';
    }
  }else{
    // 種別の台帳が渡ってきていないときは分けられない。黙って片側だけ出すと
    // 「AT機に全機種が並ぶ」ことになるので、1つにまとめて理由を書く。
    h+=aiMachineHtml(idx,base,"全機種 TOP10",null)
...
function aiPaintDais(nm){
  ...
  const rows=(pred
    ...
    : aiRows(aiCollect(idx,d=>String(d),(d,m)=>((m.n||"")===nm)),
             AI_MIN_DAI_N,AI_MIN_DAY)).slice(0,10);
  if(!rows.length){
  ...
  h+='<table class="ai-t ai-t6">'
    +aiTh([{t:"台番"},pred?{t:"予想",k:"pred"}:{t:"出率",k:"rav"},
           {t:"出率中央",k:"md"},{t:"総差枚",k:"vsum"},
           {t:"平均差枚",k:"vav"},{t:"母数",k:"n"}])
    +'<tbody>'
    +rows.map((x,i)=>{
      ...
      return '<tr><td class="ai-k"><span class="ai-rk">'+(i+1)+'</span>'
        ...
        +(pred?('<em>内訳: '+esc(x.why||"")+'</em>'):"")
        +(kuse.length?('<em>この日の癖: '+esc(kuse.join(" ／ "))+'</em>'):"")+'</td>'
        +(pred?('<td class="'+aiCl(x.pred==null?null:x.pred-100)+'">'+aiPct(x.pred)+'</td>')
              :('<td class="'+aiCl(x.rav==null?null:x.rav-100)+'">'+aiPct(x.rav)+'</td>'))
        +'<td class="'+aiCl(x.md==null?null:x.md-100)+'">'+aiPct(x.md)+'</td>'
        +'<td class="'+aiCl(x.vsum)+'">'+aiSv(x.vsum)+'</td>'
