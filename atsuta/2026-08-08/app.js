...
function buildDocs(){
  ...
  const sec=(key,label,inner,open)=>{
    if(!inner) return "";
    const [head,note]=cut(label);
    return '<details class="dsec"'+(open?" open":"")+' data-k="'+key+'">'
      +'<summary>'+esc(head)+'</summary><div class="dbody">'
      +(note?'<div class="dnote">'+esc(note)+'</div>':"")+inner
  ...
  if(D.legend&&D.legend.rows&&D.legend.rows.length){
    html+=sec("legend",D.legend.t||"凡例",
      ...
      +'<button class="sb" id="markBtn" type="button">記号を出す</button></div>',true);
  }
  ...
  if(D.rates&&D.rates.rows&&D.rates.rows.length){
    // 横に7列あると幅に入らないので、1機種=1枚のカードにして縦に積む
    // (見出しは行の中に小さく添える。表の横スクロールを作らないための形)。
    const c=D.rates.cols||[];
    html+=sec("rates",D.rates.t||"機種別出率",
      '<ol class="drate">'+D.rates.rows.map((r,i)=>{
        const v=r.v||[], fc=r.fc||[];
        return '<li><span class="dr-i">'+(i+1)+'</span>'
          +'<span class="dr-n">'+esc(v[0])+'</span>'
...
function showMove(from,to,name){
  ...
  ov.id="mvArrow";
  ov.setAttribute("width",board.offsetWidth);
  ov.setAttribute("height",board.offsetHeight);
  ov.setAttribute("viewBox","0 0 "+board.offsetWidth+" "+board.offsetHeight);
...
function lightZoom(a,barH){
  ...
  return Math.max(1,Math.min(ZOOM_IN,Math.min(availW/bw,availH/bh)));
}
