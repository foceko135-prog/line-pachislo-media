...
function fitTabbar(){
  ...
  if(ph){
    ph.style.bottom=(gap+bh+12)+"px";
    ph.style.left=(vv.offsetLeft+12)+"px";
  ...
  if(pl){
    pl.style.bottom=(gap+bh+12+52)+"px";
    pl.style.left=(vv.offsetLeft+12)+"px";
...
function neraiLabel(){
  return chipsOn("fNer").length?"／狙い方: 水曜プラス＋他曜日は凹み":"";
}
...
(()=>{
  ...
  box.addEventListener("click",e=>{
    ...
    if(b.classList.contains("on")&&!perNow()) setPer("fPer","all");
  });
})();
...
function repIntent(cur){
  ...
  if(cur.ner&&cur.ner.length) a.push("水曜には出て、他の曜日は凹んでいる台");
