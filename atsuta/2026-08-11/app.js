...
// 下部バーの期間チップ・カードのグラフ期間・絞り込みの出率の期間の3か所を一度に直す。
...
(()=>{
  const put=()=>{
    ...
    document.querySelectorAll('.pchip[data-p="single"],.mchip[data-w="1"],'
      +'#fRper .ch[data-v="single"],#fVper .ch[data-v="single"]')
...
function buildDocs(){
  ...
  if(TK.days&&TK.days.length){
    ...
    html+=sec("toku","特定日（キャラ誕・記念日）",
      '<table class="dtoku">'+COLS+'<tbody>'+rows+'</tbody></table>'+cyc,false,
...
function filterStats(dai,rper,vper){
  const m=DATA.machines[dai]; if(!m)return null;
  return {name:m.n, sum:sumForDai(dai,vper), rate:rateForDai(dai,rper)};
}
...
// いま選んでいる差枚の期間。何も選ばれていなければ直近7日(いちばんよく使う)。
function vperNow(){
  const b=document.querySelector("#fVper .ch.on");
...
// いま選んでいる出率の期間。何も選ばれていなければ3週間(それまでの挙動)。
function rperNow(){
  const b=document.querySelector("#fRper .ch.on");
  return b?b.dataset.v:"nd21";
}
...
// 差枚・出率の期間は**1つだけ**選ぶ(2026-08-11)。末尾・位置区分・曜日・特定日は
// 複数選べるトグルだが、こちらは「どの期間で見るか」なので同時に2つは成り立たない。
["fVper","fRper"].forEach(id=>{
  const box=document.getElementById(id);
  if(!box) return;
  box.addEventListener("click",e=>{
    const b=e.target&&e.target.closest?e.target.closest(".ch"):null;
...
// 期間の既定値。差枚=直近7日 / 出率=3週間(どちらもそれまでの固定値と同じ)。
const PER_DEF={fVper:"last7", fRper:"nd21"};
...
function setRper(v){ setPer("fRper",v); }
function setVper(v){ setPer("fVper",v); }
...
document.querySelectorAll(".fpre .pre").forEach(b=>{
  b.addEventListener("click",()=>{
    ["fVmin","fVmax","fRmin","fRmax","fDmin","fDmax"].forEach(id=>{
      const el=document.getElementById(id); if(el)el.value="";
    });
    // 差枚・出率の期間も既定へ戻してから当てる(2026-08-11)。前に選んだ期間が
    // 残っていると、同じボタンを押しても違う結果になってしまう。
    setVper(PER_DEF.fVper); setRper(PER_DEF.fRper);
    // 指定は data-set="fRmin=105,fVmin=1" 形式。**data-fRmin のような書き方は使えない**
    // (HTMLのdata属性は小文字化されるので、大文字を含むid(fRmin)と結び付かない)。
    // 期間だけは入力欄ではないので data-set="fVper=last7" の形で受ける。
    (b.dataset.set||"").split(",").forEach(kv=>{
      const [k,v]=kv.split("=");
      const key=(k||"").trim();
      if(key==="fRper"||key==="fVper"){ setPer(key,(v||"").trim()); return; }
...
document.getElementById("fApply").addEventListener("click",()=>{
  ...
  const active = [fVmin,fVmax,fRmin,fRmax,fDmin,fDmax].some(v=>v!=null)
  ...
  const rper=rperNow(), vper=vperNow();
  document.querySelectorAll(".tap").forEach(el=>{
    const dai=el.dataset.dai, st=filterStats(dai,rper,vper);
    let ok=!!st;
...
document.getElementById("fClear").addEventListener("click",()=>{
  ["fVmin","fVmax","fRmin","fRmax","fDmin","fDmax"]
    .forEach(id=>{ const el=document.getElementById(id); if(el)el.value=""; });
  document.querySelectorAll("#fSue .ch,#fPos .ch,#fDow .ch,#fToku .ch")
    .forEach(b=>b.classList.remove("on"));
  setVper(PER_DEF.fVper); setRper(PER_DEF.fRper);   // 期間も既定へ(2026-08-11)
  clearHits();
...
function setCellKitai(el){
  ...
  el.innerHTML=head+'<span class="ktn">'+mk+"</span>"+h.slice(m.index);
}
...
function paintHiru(dai){
  ...
  // 推定設定は小数第2位まで(2026-08-11・谷川氏指示「①〜⑥から通常数字で小数点第二まで」)。
  const c3=(n,body,se)=>"<td>"+n+'</td><td class="hc">'+body+'</td><td class="hs">'
    +(se==null?"":'<span class="hb">'+se.toFixed(2)+"</span>")+"</td>";
  const cell=(o,kind)=>{
    if(!o) return c3("−","","");
    ...
    if(kind==="kitai"){
      ...
      return '<td></td><td class="hc"></td><td class="hs">'
        +(e?('<span class="hb">'+e.toFixed(2)+"</span>"):"")+"</td>";
    }
  ...
  if(A&&A.pct&&st&&st.labels){
    const pr=(o,lb)=>{
      ...
      return "<tr><th>"+lb+"</th>"+o.pct.map(x=>"<td>"+x.toFixed(1)+"%</td>").join("")
        +"<td>"+(o.kitai?o.kitai.toFixed(2):"−")+"</td></tr>";
    };
