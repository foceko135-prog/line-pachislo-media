...
// 出率は小数点なしで出す(2026-08-26・谷川さん指示「出率を小数点は無しで」)。
// ★100%の線をまたがせない。99.6%を四捨五入して「100%」と出すと、赤字(100%未満)
//   なのに100%に見えて読み違える。100未満は99止まり、100以上は100止まりにする。
// プラス率はこれまでどおり小数1桁(指示は出率だけ)。
const fpc=r=>{
  if(typeof r!=="number") return "−";
  let n=Math.round(r);
  if(r<100&&n>=100) n=99;
  if(r>=100&&n<100) n=100;
  return n+"%";
};
const fr=r=>fpc(r);
...
function drawChart(days,labels,CH,base,intra){
  ...
  if(intra&&gTot>0&&days.length>=4){
    ...
    // ★10〜50を足した(2026-09-02)。いちばん小さい刻みが100だったので、当日130Gの台は
    //   どの刻みでも1本も入らず目盛りが消えていた(verify_shimaheat_intra_axis が
    //   台2762/3250 で落ちていた実害)。大きい台は「7本以内」の条件で小さい刻みが
    //   自動的に外れるので、これまでの見え方は変わらない。
    for(const st of [10,20,25,50,100,200,250,500,1000,2000,2500,5000,10000]){
      const arr=[];
...
function renderDay(dai){
  ...
  try{ awCardNote(dai); }catch(e){}   // 答え合わせ中なら「当時 ○○」を添える(2026-09-03)
  buildMini(dai,m.n);
  syncMhiru();
...
function titleCands(){
  ...
  const m=DATA.machines&&DATA.machines[curDai];
  // ★候補は**実際に出る見出しと同じ文字列**にする(2026-08-27)。
  //   renderCard は窓を切り出したあと、前後の「数字が無い日」を落としてから
  //   両端の日付を書く。ここで落とさずに窓の頭をそのまま使っていたため、
  //   入替などで途中から動き出した台では候補のほうが短くなり、詰めが足りずに
  //   本物の見出しがはみ出していた(実例 台3026・3週間: 候補「8/6(水)〜」に対し
  //   実物は「8/17(月)〜」で1文字長く、12pxのまま1.3pxはみ出した)。
  //   前日まで通っていたのは日数がたまたま1桁だっただけで、元からあった穴。
  const trim=(L,d)=>{
    if(!d) return L;
    let s=0, e=d.length-1;
    while(s<=e && (!d[s] || d[s][0]==null)) s++;
    while(e>=s && (!d[e] || d[e][0]==null)) e--;
    return (s<=e && (s>0 || e<d.length-1)) ? L.slice(s,e+1) : L;
  };
  const mk=n=>{
    const cut=N-Math.min(n>0?n:N,N);
    const L=trim(DATA.labels.slice(cut), (m&&m.d)?m.d.slice(cut):null);
    if(!L.length) return "";
    const rng=(L.length>1)?(L[0]+"〜"+L[L.length-1]):L[0];
    return "台"+curDai+" "+rng+" "+L.length+"日";
  };
  const out=[mk(7),mk(NDAYS),mk(0)].filter(Boolean);
...
// ===== 画像を読み込んでいる間の「ぐるぐる」(2026-08-30・谷川さん指示
//       「読み込んでいる間はぐるぐる回るロード中というのがわかるものを出して欲しい。
//        この筐体画像に限らず、他の画像読み込みの際もそのようにして」) =====
// 作り: **入れ物を新しく作らない**。画像の親に印(.ld-on)を付けて、その親の ::after に
//   回る輪を描くだけ。<img> は ::after を持てないので親を借りる。DOMを足さないので
//   どの画像でも並びが崩れない。読めた／失敗したら印を外す。
// ★付けるのは「その親に画像が1枚だけ」のときに限る。輪は親の真ん中に出るので、
//   画像が並んでいる入れ物(スペックの画像一覧など)に付けるとどれの輪か分からない。
// ★親が position:static のときだけ relative にして、外すときに元へ戻す
//   (勝手に position を残すと、後から絶対配置する物の基準がずれる)。
function ldMark(im){
  if(!im||im.dataset.ld) return;
  if(im.complete&&im.naturalWidth) return;          // すでに読めている
  const p=im.parentElement;
  if(!p||p.childElementCount>6) return;
  if(p.querySelectorAll("img").length!==1) return;
  im.dataset.ld="1";
  if(getComputedStyle(p).position==="static"){ p.dataset.ldpos="1"; p.style.position="relative"; }
  p.classList.add("ld-on");
}
function ldClear(im){
  if(!im||!im.dataset.ld) return;
  delete im.dataset.ld;
  const p=im.parentElement;
  if(!p) return;
  p.classList.remove("ld-on");
  if(p.dataset.ldpos){ delete p.dataset.ldpos; p.style.position=""; }
}
// load/error は上へ伝わらないので**捕まえる側(capture)**で拾う=1組で全部の画像に効く
document.addEventListener("load",e=>{
  if(e.target&&e.target.tagName==="IMG") ldClear(e.target); },true);
document.addEventListener("error",e=>{
  if(e.target&&e.target.tagName==="IMG") ldClear(e.target); },true);
// 後から足される画像(カード・資料・拡大表示)を拾う。盤面のセルは画像を持たないので軽い。
(function(){
  const scan=n=>{
    if(n.nodeType!==1) return;
    if(n.tagName==="IMG") ldMark(n);
    else if(n.querySelectorAll) n.querySelectorAll("img").forEach(ldMark);
  };
  new MutationObserver(ms=>{ ms.forEach(m=>m.addedNodes.forEach(scan)); })
    .observe(document.documentElement,{childList:true,subtree:true});
  // src を後から差し替える物(拡大表示の #photoImg など)にも効かせる
  new MutationObserver(ms=>{ ms.forEach(m=>{
    const t=m.target;
    if(t&&t.tagName==="IMG"&&m.attributeName==="src") ldMark(t); }); })
...
function buildMini(dai,mname){
  ...
  kishuLoad().then(()=>{
    ...
    // ★読み込んでいる間は**枠を置いてぐるぐるを出す**(2026-08-30・谷川さん指示)。
    //   輪は共通の仕組み(ldMark/.ld-on)が親=この枠に付ける。
    // ★読めなかったときは**枠ごと消す**(谷川さん報告「左上隅の筐体の画像が
    //   表示されない」＝空の四角い枠だけが残っていた)。#mini は box-shadow で
    //   1pxの輪郭を持つので、中身が無いまま残すと壊れて見える。
    // ★1回だけ取り直す。ホールの中は電波が弱く、1回目に取りこぼすことがある。
    let tried=0;
    im.onerror=()=>{
      if(tried++){ box.remove(); return; }   // 2回目も駄目なら枠ごと消す
      setTimeout(()=>{ im.src=f+(f.indexOf("?")<0?"?r=1":"&r=1"); },1200);
    };
    im.src=f;
    box.appendChild(im);
...
// カードの位置をJSが握っている最中かどうか(2026-09-02に swanim を追加)。
//   dragging … 指でつまんでいる間(縦に閉じる/横に送る)
//   swanim   … 左右に送るときの「出ていく→入ってくる」の動きの最中
// この間は positionOverlayCard が transform を上書きしない。上書きすると
// 描き直し(fitCard)の瞬間に中央へ引き戻されて、動きが途中で切れる。
function cardHeld(card){
  return card.classList.contains("dragging")||card.classList.contains("swanim");
}
function positionOverlayCard(card,maxW,sheet){
  ...
  if(!vv||!vv.width||!vv.height){
    if(sheet){
      ...
      card.style.width=""; if(!cardHeld(card)) card.style.transform="";
  ...
  if(sheet){
    ...
    if(!cardHeld(card)) card.style.transform=`scale(${1/vv.scale})`;
...
function onVVChange(){
  ...
  vvTimer=setTimeout(()=>{
    ...
    // 資料の中の欄を打っている最中にキーボードが出てきたとき(2026-09-09)。
    // 押した直後の測り直し(60〜820ms)より遅れて出る端末のための保険。
    const dp=document.getElementById("docsPanel"), ae=document.activeElement;
    if(dp&&!dp.hidden&&ae&&ae.tagName==="INPUT"&&dp.contains(ae)) docsKbApply();
  },150);
}
...
// ===== 狙い方の実績(2026-08-27・谷川さん指示「資料に狙い方の実績を追加して。
//       過去の80台毎の結果も全てそれぞれみれるようにしておいて」) =====
// 中身は build_nerai_rec.py が毎晩作り直す nrec.<hash>.json(約52KB)。
//   通算のまとめ ／ 週ごとの成績 ／ その水曜に選んだ80台を1台ずつ。
// ★数字は画面で作り直さない。PDF(nerai_ledger_pdf/nerai_day_pdf)とDiscordが出す値と
//   同じものを並べているだけ。ここで丸め直すと紙と画面で食い違う。
// ★毎日まるごと落ちる last7.data.js には入れない。**節を開いたときだけ**落とす。
let NREC=null, nrecReq=null, nrecWeek=null;
function nrecLoad(){
  if(NREC) return Promise.resolve(NREC);
  if(typeof SHIMA==="undefined"||!SHIMA||!SHIMA.nrecUrl) return Promise.resolve(null);
  if(nrecReq) return nrecReq;
  nrecReq=fetch(asrc(SHIMA.nrecUrl),{cache:"default"})
    .then(r=>r.ok?r.json():null)
    .then(j=>{ NREC=j; return j; })
    .catch(()=>{ nrecReq=null; return null; });
  return nrecReq;
}
const ncE=s=>String(s==null?"":s).replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]));
const ncSv=v=>(typeof v==="number")?((v>0?"+":"")+v.toLocaleString()):"−";
const ncCl=v=>(typeof v!=="number")?"":(v>0?"nc-p":(v<0?"nc-m":""));
// プラス率は小数1桁でそろえる(出率とちがい指示は無いが、55.0を「55%」と出すと
// 隣の「42.5%」と桁がそろわず、表の中で数字が踊って見える・2026-08-27)
const ncPt=v=>(typeof v==="number")?(v.toFixed(1)+"%"):"−";
// 節の中身をぜんぶ組む(通算 → 週ごと → 順位帯 → 週を選ぶ → その週の80台)
function nrecHtml(D){
  const S=D.sum||{}, W=D.weeks||[];
  const card=(t,v,c)=>'<div class="nc-c"><span class="nc-ct">'+ncE(t)+'</span>'
    +'<b class="'+c+'">'+ncE(v)+'</b></div>';
  let h='<div class="dnote">直近7日で凹んだAT機の上位'+D.topk+'台を、'
    +'その日より前の記録だけで毎週選び直し、実際に打った当日の結果と'
    +'突き合わせたものです（先読みなし）。'+ncE(D.w)+'曜'+S.n+'週ぶん。</div>';
  h+='<div class="nc-cards">'
    +card("選んだ"+D.topk+"台",ncSv(S.in&&S.in.a)+"枚",ncCl(S.in&&S.in.a))
    +card("リスト外のAT機",ncSv(S.out&&S.out.a)+"枚",ncCl(S.out&&S.out.a))
    +card("その差",ncSv(S.dif)+"枚",ncCl(S.dif))
    +card("上回った週",S.win+" / "+S.n,"")
    +'</div>';
  h+='<div class="nc-sub">数字は1台1日の平均差枚。同じ日の「リストに入らなかったAT機」と'
    +'くらべています（全館ぜんぶの平均は '+ncSv(S.all&&S.all.a)+'枚）。</div>';
  h+='<div class="tz-dsub">週ごとの成績</div>'
    +'<div class="dtwrap"><table class="dtbl"><thead><tr>'
    +'<th class="dt-d">'+ncE(D.w)+'曜</th><th>選んだ'+D.topk+'台</th>'
    +'<th>リスト外AT</th><th>差</th><th>プラス率 選</th><th>プラス率 外</th>'
    +'<th>選の出率</th></tr></thead><tbody>'
    +W.map(w=>'<tr><td class="dt-d">'+ncE(w.d)+'</td>'
      +'<td class="'+ncCl(w.in.a)+'">'+ncSv(w.in.a)+'</td>'
      +'<td class="'+ncCl(w.out.a)+'">'+ncSv(w.out.a)+'</td>'
      +'<td class="'+ncCl(w.dif)+'">'+ncSv(w.dif)+'</td>'
      +'<td>'+ncPt(w.in.p)+'</td><td>'+ncPt(w.out.p)+'</td>'
      +'<td class="'+((typeof w.in.r==="number"&&w.in.r<100)?"nc-m":"nc-p")+'">'
      +fpc(w.in.r)+'</td></tr>').join("")
    +'<tr class="dt-sum"><td class="dt-d">通算</td>'
    +'<td>'+ncSv(S.in&&S.in.a)+'</td><td>'+ncSv(S.out&&S.out.a)+'</td>'
    +'<td>'+ncSv(S.dif)+'</td><td>'+ncPt(S.in&&S.in.p)+'</td>'
    +'<td>'+ncPt(S.out&&S.out.p)+'</td>'
    // 通算の出率(2026-09-09)。週ごとの%を平均したものではなく、
    // 週をまたいだG数と差枚の合計から出し直した値(build_nerai_rec.totrate)
    +'<td class="'+((S.in&&typeof S.in.r==="number"&&S.in.r<100)?"nc-m":"nc-p")+'">'
    +fpc(S.in&&S.in.r)+'</td></tr>'
    +'</tbody></table></div>';
  // 順位帯(2026-08-27に作り直し・谷川さん指示「順位帯の棒にプラス率と中央値も並べる」)。
  // 平均差枚だけだと一撃の大きさに振られて上下が読めない。当たりやすさ(プラス率)と
  // 真ん中の台(中央値)を同じ行に添えて、毎週この目で追えるようにする。
  const bs=S.b||[];
  if(bs.length){
    const mx=Math.max.apply(null,bs.map(x=>Math.abs(x.a||0)))||1;
    h+='<div class="tz-dsub">順位帯ごとの通算</div>'
      +'<ul class="nc-band">'+bs.map(x=>'<li>'
        +'<div class="nc-br"><span class="nc-bl">'+x.lo+'〜'+x.hi+'番目</span>'
        +'<span class="nc-bb"><i style="width:'
        +Math.max(4,Math.round(Math.abs(x.a||0)/mx*100))+'%"></i></span>'
        +'<b class="'+ncCl(x.a)+'">'+ncSv(x.a)+'枚</b></div>'
        +'<div class="nc-b2">プラス率 '+ncPt(x.p)
        +' ／ 中央値 <span class="'+ncCl(x.m)+'">'+ncSv(x.m)+'枚</span>'
        +(x.n?(' ／ '+x.n+'台日'):"")+'</div>'
        +'</li>').join("")+'</ul>'
      +'<div class="nc-sub">棒は平均差枚。平均は一撃の大きさに振られるので、'
      +'当たりやすさ（プラス率）と真ん中の台（中央値）も並べています。'
      +'上位ほど良いという傾向はありません。上から順に取りにいく必要はなく、'
      +'空いている台で構いません。</div>';
    // 見張り(2026-08-27)。上位20台だけプラス率が低い形が出ているが、まだ再現していない。
    // 「効いている」と読み違えないよう、前半と後半の両方を必ず並べて出す。
    const wt=S.watch;
    if(wt&&wt.h&&wt.h.length===2){
      h+='<div class="nc-sub nc-watch">見張り中：上位20台のプラス率が'
        +'21〜'+D.topk+'番より低かった週は '+wt.low+' / '+wt.n+'。'
        +'ただし '+ncE(wt.h[0].d)+' では逆（'+ncPt(wt.h[0].a)+' 対 '+ncPt(wt.h[0].b)+'）、'
        +ncE(wt.h[1].d)+' では '+ncPt(wt.h[1].a)+' 対 '+ncPt(wt.h[1].b)+'。'
        +'前半と後半で向きが変わるうちは運の範囲なので、順位帯で取捨はしません。'
        +'両方が同じ向きにそろったときが、狙い方を変えるタイミングです。</div>';
    }
  }
  h+='<div class="tz-dsub">1台ずつの結果を見る'+ncE(D.w)+'曜を選ぶ</div>'
    +'<div class="nc-ws">'+W.map((w,i)=>'<button type="button" class="nc-wb'
      +(i===nrecWeek?" is-on":"")+'" data-i="'+i+'">'+ncE(w.d)+'</button>').join("")
    +'</div><div id="nrecWk">'+nrecWeekHtml(D)+'</div>';
  return h;
}
// 選んだ週の80台を1台ずつ
function nrecWeekHtml(D){
  const w=(D.weeks||[])[nrecWeek];
  if(!w) return "";
  const rows=w.rows||[];
  // その日いちばん出た台。順位と当たりの強弱が結びつかないことの実例になる。
  let top=null;
  rows.forEach(r=>{ if(typeof r[4]==="number"&&(!top||r[4]>top[4])) top=r; });
  let h='<div class="nc-wh">'+ncE(w.d)+'（'+ncE(w.w)+'）に選んだ'+rows.length+'台</div>'
    +'<div class="nc-wsum">平均 <b class="'+ncCl(w.in.a)+'">'+ncSv(w.in.a)+'枚</b>'
    +' ／ プラス率 '+ncPt(w.in.p)+' ／ 出率 '+fpc(w.in.r)
    +' ／ リスト外AT <b class="'+ncCl(w.out.a)+'">'+ncSv(w.out.a)+'枚</b>'
    +' ／ 差 <b class="'+ncCl(w.dif)+'">'+ncSv(w.dif)+'枚</b></div>';
  if(top){
    h+='<div class="nc-sub">この日いちばん出たのは 台'+ncE(top[1])+'（'
      +ncE(top[2])+'・リスト'+top[0]+'番目）で '+ncSv(top[4])+'枚。</div>';
  }
  // 当日の回転数(G)も出す(2026-08-27・谷川さん指示「当日回転Gもいれてください」)。
  // 差枚だけだと「回っていないから減っていない」台と「回して出た」台の区別が付かない。
  h+='<div class="dtwrap"><table class="dtbl"><thead><tr>'
    +'<th class="dt-d">順</th><th>台番</th><th class="nc-km">機種</th>'
    +'<th>直近7日</th><th>当日差枚</th><th>当日G</th><th>出率</th>'
    +'</tr></thead><tbody>'
    +rows.map(r=>'<tr><td class="dt-d">'+r[0]+'</td>'
      +'<td>'+ncE(r[1])+'</td><td class="nc-km">'+ncE(r[2])+'</td>'
      +'<td class="nc-m">'+(typeof r[3]==="number"?r[3].toLocaleString():"−")+'</td>'
      +'<td class="'+ncCl(r[4])+'">'+ncSv(r[4])+'</td>'
      +'<td class="nc-g">'+(typeof r[5]==="number"?r[5].toLocaleString():"−")+'</td>'
      +'<td class="'+((typeof r[6]==="number"&&r[6]<100)?"nc-m":"nc-p")+'">'
      +fpc(r[6])+'</td></tr>').join("")
    +'</tbody></table></div>'
    +'<div class="nc-sub">「直近7日」はその台を選んだときの平均差枚（マイナスが'
    +'深いほど上の順位）。「当日差枚」と「当日G」はその'+ncE(w.w)+'曜1日ぶんの'
    +'実績で、出率は (3G＋差枚)÷3G です。</div>';
  return h;
}
function nrecPaintWeek(){
  const h=document.getElementById("nrecWk");
  if(!h||!NREC) return;
  h.innerHTML=nrecWeekHtml(NREC);
  document.querySelectorAll("#nrecBody .nc-wb").forEach(b=>{
    b.classList.toggle("is-on",Number(b.dataset.i)===nrecWeek);
  });
}
// 節を開いたときに読み込んで描く。資料は作り直されることがあるので、
// 器の有無ではなく**中身が入っているか**(data-done)で二度描きを避ける。
function nrecFill(){
  const el=document.getElementById("nrecBody");
  if(!el||el.dataset.done==="1") return;
  nrecLoad().then(j=>{
    const e2=document.getElementById("nrecBody");
    if(!e2||e2.dataset.done==="1") return;
    if(!j||!(j.weeks||[]).length){
      e2.innerHTML='<div class="dempty">狙い方の実績はまだ作られていません。</div>';
      return;
    }
    // 既定は**いちばん新しい週**(いま知りたいのは直近の水曜のはずなので)
    if(nrecWeek==null||nrecWeek>=j.weeks.length) nrecWeek=j.weeks.length-1;
    e2.dataset.done="1";
    e2.innerHTML=nrecHtml(j);
  });
}
function buildDocs(){
  ...
  // 狙い方の実績(2026-08-27・谷川さん指示)。中身は節を開いたときに落とす(約52KB)ので、
  // ここでは器だけ置く。落とす・描くは nrecFill() が受け持つ。
  if(typeof SHIMA!=="undefined"&&SHIMA&&SHIMA.nrecUrl){
    html+=sec("nrec","狙い方の実績",
      '<div id="nrecBody" class="nrec"><div class="dempty">読み込み中…</div></div>');
  }
  ...
  if(R&&R.rows&&R.rows.length){
    ...
    const c=R.cols||[];
    // ★列は名前で引く(2026-08-28)。以前は v[2]=出率 のように位置で決め打ちしていて、
    //   プラス・勝率の2列を足したときに全部ずれた(機種名が数字になった)。
    const IX=(n,d)=>{const i=c.indexOf(n); return i<0?d:i;};
    const IX_CNT=IX('台数',1), IX_RATE=IX('出率',2), IX_YEN=IX('収支',6);
    const IX_G=(c.indexOf('平均G')>=0?c.indexOf('平均G'):IX('平均G/台日',4));
    const IX_AVG=(c.indexOf('差枚/台')>=0?c.indexOf('差枚/台'):IX('平均差枚/台日',5));
    const IX_PLUS=c.indexOf('プラス'), IX_WIN=c.indexOf('勝率');
    ...
    html+=sec("rates",rt,
      '<ol class="drate">'+R.rows.map((r,i)=>{
        ...
        return '<li><span class="dr-i">'+(i+1)+'</span>'
          ...
          +'<span class="dr-r" style="color:'+esc(fc[0]||"inherit")+'">'+esc(v[IX_RATE])+'</span>'
          ...
          +'<span class="dr-s">'+esc(v[IX_CNT])+(r.nd?'・'+r.nd+'日間':"")
          +(r.td?'（延べ'+r.td.toLocaleString()+'台日）':"")
          +((IX_PLUS>=0&&IX_WIN>=0)?('・勝率 '+esc(v[IX_WIN])+'（プラス '+esc(v[IX_PLUS])+'）'):"")
          +'・'+esc(c[IX_G]||"平均G")+' '+esc(v[IX_G])
          +'・'+esc(c[IX_AVG]||"平均差枚")+' '+esc(v[IX_AVG])
          +(typeof r.sv==="number"
            ?('・累計差枚 <b class="'+(r.sv>0?"dr-p":(r.sv<0?"dr-m":""))+'">'
              +((r.sv>0?"+":"")+r.sv.toLocaleString())+"</b>"):"")+'</span>'
          +'<span class="dr-y" style="color:'+esc(fc[1]||"inherit")+'">'+esc(v[IX_YEN])+'</span></li>';
      }).join("")+'</ol>',false,
      {head:rbase+"【"+pnm+"】",
    ...
    if(TR.osusume&&TR.osusume.length){
      const rows=TR.osusume.map(e=>{
        ...
        return '<tr><td class="tz-d">'+esc(e.period)+'<span>'+esc(e.days||"")+'日</span></td>'
          ...
          // 判定は3段(2026-08-28・谷川さん指摘「―の横棒線が弱いのか通常なのか
          // わからないから、何か入れるようにして」)。★強い/△ 並/× 弱い で色も分ける。
          +(e.judge?'<span class="tz-tag'
            +(/強/.test(e.judge)?" tz-hot":(/弱/.test(e.judge)?" tz-low"
              :(/並/.test(e.judge)?" tz-mid":"")))+'">'
            +esc(e.judge)+'</span>':"")
      ...
      html+=sec("osusume","おすすめ機種",
        ...
        //   2026-08-28: 欄をさらに広げ、それでも入らないときは**切らずに折り返す**
        //   (谷川さん報告「日付見えるようにして」。実機は検証環境より字が広く、
        //    「8/11〜8/15」が「8/11〜8/…」と切れていた)。目印が `dtori-w`。
        '<table class="dtori dtori-w"><colgroup><col class="tzc-dw"><col></colgroup>'
        +'<tbody>'+rows+'</tbody></table>',false,
        // ★説明は**実際の判定の式**に合わせる(2026-08-28)。それまで「どれもが上回った」と
        //   書いていたが、判定に使っているのは**フロア比だけ**で、条件は1つ。
        {pre:["P-WORLD店舗ページの「おすすめ」を毎晩控えて、その期間の実績を突き合わせたもの",
              "判定はフロア比（同じ期間の全機種平均との差）で3段。"
              +"★強い＝+100枚超／△ 並＝±100枚以内／× 弱い＝-100枚未満"]});
    }
  }
  ...
  if(!host.dataset.upTap){ host.dataset.upTap="1";
    ...
    // 狙い方の実績は**開いたときに**中身を落とす(2026-08-27)。
    // ★toggle は上へ伝わらないので capture(第3引数 true)で拾う。
    host.addEventListener("toggle",e=>{
      const d=e.target;
      if(!d||!d.dataset||d.dataset.k!=="nrec"||!d.open) return;
      nrecFill();
    },true);
    // 見る週を切り替える(過去の水曜は全部そのまま残してある)
    host.addEventListener("click",e=>{
      const b=e.target.closest(".nc-wb");
      if(!b) return;
      nrecWeek=Number(b.dataset.i);
      nrecPaintWeek();
    });
...
function setView(v,keepScroll){
  ...
  if(v==="island"){
    ...
    //   スマホの縦向きは従来どおり(全体を入れると台番が読めない大きさになるので高さ合わせ)。
    // ★スマホを横向きにしたときも全体を入れる(2026-08-27・谷川氏指示
    //   「横向きにしたときに島図全体表示できるようにして」)。横向きは幅が広く高さが浅いので、
    //   幅合わせのままだと下の島が下部バーの向こうへ隠れて切れていた(実機の画面で確認)。
    //   iPhoneは横向きでも幅844px程度で PC_WIDE(900) に届かずスマホ扱いになるため、
    //   幅の絶対値とは別に「横長かどうか」でも全体フィットへ入れる。
    const yoko=window.innerWidth>window.innerHeight;
    sc=(window.innerWidth>=PC_WIDE||yoko)
       ? Math.min(availW/w, availH/h)
       : Math.max(availW/w, Math.min(availH/h, 0.6));   // 縦向きは高さフィット(幅フィットを下回らない)
  }else{
...
(()=>{
  ...
  // ★横向きのスマホでは寄せない(2026-08-27)。幅だけ見ると PC_WIDE を超える端末があり
  //   (iPhoneのPro Maxは横向き932px)、そのままだと「横向きは全体を見せる」という
  //   指示と正面からぶつかる。窓の高さが浅いものは横向きの携帯とみなして素通りする。
...
function tkQApply(){
  ...
  // 打つたびに行が消えてページが縮み、欄が指の下から逃げるので位置を取り直す
  // (2026-09-09。キーボードが出ているときだけ効く)
  if(typeof docsKbApply==="function"&&document.body.classList.contains("kb-on")){
    docsKbApply();
  }
}
...
const DOCS_GROUPS=[
  ...
  {t:"店の数字",         k:["rates","daily","nrec","osusume"]},
...
// 資料の中の検索窓(特定日)を押すと画面が見えなくなる件(2026-09-09・谷川さん報告
// 「特定日の検索窓タップすると画面がみれなくなる」)。
// iOSはキーボードが出ると可視領域が半分以下になる。そこへ下部バー4段(169px=画面の22%)が
// そのまま重なるので、打ち込む欄も一覧も残らない。#fBody(絞り込み)には同じ手当てが
// 入っていたが、資料パネルは素のページなので素通しだった。
// 対処は2つ。(1)打っている間だけ下部バーを畳む(localStorageには覚えさせない)。
//           (2)キーボードが出きった後に、欄を見えている領域の上の方へ持ってくる。
// ★キーボードの動きは非同期なので、1回だけ測ると縮む前の値で計算してしまう。
//   #fBody と同じく 60/260/520/820ms の4回測り直す。
// ★位置は visualViewport の offsetTop を引いて「見えている領域の中での位置」に直す。
//   getBoundingClientRect はレイアウト側の座標なので、そのまま使うとキーボードで
//   ずれた分だけ狙いを外す。
let dKbTimers=[], dKbBarWasOpen=false;
// ★キーボードが本当に出ているときだけ効かせる(2026-09-09)。欄に focus が入っただけで
//   バーを畳むと、PCや検証(headless)のように**キーボードが出ない環境**でも
//   画面が動いてしまう(verify_shimaheat_tokuq の「節の頭に居るうちは出ない」が落ちた)。
//   レイアウト側の高さ(layoutH)は変わらず、見えている高さ(visualViewport)だけが
//   キーボードで縮むので、その差で判定する。
function docsKbUp(){
  const vv=window.visualViewport;
  return !!vv && (layoutH()-vv.height)>100;
}
function docsKbApply(){
  const a=document.activeElement, p=document.getElementById("docsPanel");
  if(!a||a.tagName!=="INPUT"||!p||!p.contains(a)) return;
  if(!docsKbUp()) return;
  if(!document.body.classList.contains("kb-on")){
    const tb=document.getElementById("tabbar");
    dKbBarWasOpen=!!(tb&&!tb.classList.contains("is-min"));
    document.body.classList.add("kb-on");
    if(dKbBarWasOpen) setBar(true,false);
  }
  docsInputInView();
}
function docsInputInView(){
  const a=document.activeElement;
  const p=document.getElementById("docsPanel");
  if(!a||a.tagName!=="INPUT"||!p||!p.contains(a)) return;
  const vv=window.visualViewport, vh=vv?vv.height:window.innerHeight;
  const r=a.getBoundingClientRect();
  const top=r.top-(vv?vv.offsetTop:0);
  const want=top-Math.max(8,Math.round(vh*0.10));
  if(Math.abs(want)>4) window.scrollBy(0,want);
}
(function(){
  const p=document.getElementById("docsPanel");
  if(!p) return;
  p.addEventListener("focusin",e=>{
    if(!e.target||e.target.tagName!=="INPUT") return;
    dKbTimers.forEach(clearTimeout);
    dKbTimers=[60,260,520,820].map(ms=>setTimeout(docsKbApply,ms));
  });
  p.addEventListener("focusout",()=>{
    dKbTimers.forEach(clearTimeout);
    setTimeout(()=>{
      const a=document.activeElement;
      if(a&&a.tagName==="INPUT"&&p.contains(a)) return;   // 別の欄へ移っただけ
      document.body.classList.remove("kb-on");
      if(dKbBarWasOpen){ dKbBarWasOpen=false; setBar(false,false); }
    },120);
  });
})();
  ...
  const cell=(o,kind)=>{
    ...
    if(kind==="v"){
      ...
      return '<td class="'+(o.v>0?"plus":(o.v<0?"minus":""))+'">'+sgn(o.v)
        +'</td><td class="hc">'+(rr!=null?fpc(rr):"")+'</td><td class="hs"></td>';
    }
...
function paintDetail(dai){
  ...
  const rows=[
    ...
    // 出率だけ小数なし(2026-08-26)。すぐ下のプラス率は小数1桁のまま
    ["出率", fpc(sr), fpc(B.rate)],
...
["fSue","fPos","fDow","fToku","fSet","fNer"].forEach(id=>{
  ...
  box.addEventListener("click",e=>{
    ...
    // ★入れ子のチップ群は自前の受け口が面倒を見るので、ここでは触らない
    //   (2026-08-23夕。#fNer の中に「狙う台数」を入れたとき、この汎用の切替が
    //    台数チップまで拾って、#fRecN 側が付けた .on を即座に打ち消していた=
    //    40台/150台を押しても80台のままになる、という形で実際に踏んだ)
    if(b&&b.closest("#fRecNRow")) return;
    if(b&&b.closest("#fRecXRow")) return;   // 「さらに絞る」も入れ子(2026-09-10)
    // グラフ一覧も #fNer の中へ入れ子にすることがある(2026-08-24夜)。
    // いまは中に .ch を持たないが、増やしたときに同じ事故を踏まないよう外しておく。
    if(b&&b.closest("#fGraphBox")) return;
...
// ★見るのは「直下のチップ」だけ(2026-08-23夕)。狙い方の中に「狙う台数」の欄を
//   入れ子にしたため、子孫まで拾うと台数(80など)が狙い方の選択として混ざる。
//   実測でどのチップ群も .ch は直下に並んでいる(fDow/fToku/fSue/fPos/fSet/fNer/
//   fPer/fRecN の8つで「全部の数=直下の数」を確認)ので、直下限定にして支障はない。
...
const NERAI_REC_N = 80;      // 既定の台数(検証で最も安定していた数)
const NERAI_REC_MIN = 4;     // 直近7日のうち、記録がこれ未満の台は判定しない
// 台数は画面から選べる(2026-08-23・谷川氏指示「混んでる日は150台まで広げたい」)。
// ★選べるのは40/80/150の3つだけ。sim_wed_ways.py で実際に検証したのがこの3つで、
//   それ以外の台数は成績の裏付けが無いため出さない。数字はすべて同じ水曜9日
//   (6/24〜8/19)・46枚貸し・5.06枚交換・現金1割の前提。
const NERAI_REC_STAT = {
  40:{y:"+10,833円", lo:"-742円", hi:"+24,469円", pct:"26.0%",
      s:"水曜9日で1日あたり +10,833円／無作為に選ぶより上位26.0%。平均はいちばん高い"
        +"ですが、日ごとのぶれも大きく（95%の幅 -742〜+24,469円）マイナスの日もあり得ます。"},
  80:{y:"+10,417円", lo:"+2,052円", hi:"+17,931円", pct:"12.3%",
      s:"水曜9日で1日あたり +10,417円／無作為に選ぶより上位12.3%。95%の幅は "
        +"+2,052〜+17,931円 でゼロをまたぎません。63通り試した中でいちばん良い選び方です。"},
  150:{y:"+5,883円", lo:"-1,155円", hi:"+11,606円", pct:"15.9%",
      s:"水曜9日で1日あたり +5,883円／無作為に選ぶより上位15.9%。台数を広げるぶん"
        +"1台あたりは下がります（95%の幅 -1,155〜+11,606円）。80台が埋まる混んだ日に。"}
};
// いま選ばれている台数。選ばれていなければ既定(80)へ倒す。
function recNNow(){
  const b=document.querySelector("#fRecN .ch.on");
  const n=b?parseInt(b.dataset.v,10):NERAI_REC_N;
  return NERAI_REC_STAT[n]?n:NERAI_REC_N;
}
function setRecN(n){
  const box=document.getElementById("fRecN"); if(!box) return;
  box.querySelectorAll(".ch").forEach(x=>{
    x.classList.toggle("on", parseInt(x.dataset.v,10)===n);
  });
}
// 出したばかりの欄が可視領域の外にあるなら、そこまで送る(2026-08-23)。
// ★谷川氏報告「位置関係的に追加した狙う台数が見えない」。狙い方のタイルは2枚で
//   640pxあり、押した直後の台数チップは #fBody の可視領域(514px)の外に出る
//   (実測: チップ top=656 に対し可視の下端 598 → 1pxも見えない)。しかも狙い方を
//   選ぶと下部バーが 120→156px に伸びて可視領域がさらに縮む。**出すだけでは
//   気づけない**ので、出したことを画面の動きで見せる。
function scrollRowIntoBody(row){
  const body=document.getElementById("fBody");
  if(!body||!row||row.hidden) return;
  const rb=row.getBoundingClientRect(), bb=body.getBoundingClientRect();
  const over=rb.bottom-(bb.bottom-6);            // 下にはみ出している量
  const up=(bb.top+6)-rb.top;                    // 上にはみ出している量
  let dy=0;
  // 下へ送るときも、欄の上端(見出し「狙う台数」)は必ず可視領域に残す
  if(over>0) dy=Math.min(over, rb.top-(bb.top+6));
  else if(up>0) dy=-up;
  if(Math.abs(dy)<2) return;
  // 動きを減らす設定のときは一気に送る(この画面の他の演出と同じ扱い)
  const slow=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  try{ body.scrollTo({top:body.scrollTop+dy, behavior:slow?"auto":"smooth"}); }
  catch(_){ body.scrollTop+=dy; }
}
// 台数の欄の出し入れと、タイルの文言の入れ替え。
// ★狙い方を選んでいないときは出さない(効かない選択肢を置かない)。
// reveal=true は「谷川さんがいま狙い方を押した」とき。**そのときだけ**欄まで送る
// (パネルを開き直しただけで下へ飛ぶと、上の説明を読む前に飛ばされたように見える)。
function syncRecN(reveal){
  const on=chipsOn("fNer").indexOf("recat")>=0;
  const row=document.getElementById("fRecNRow");
  const was=row?row.hidden:true;
  if(row) row.hidden=!on;
  if(reveal&&row&&was&&!row.hidden){
    row.classList.remove("just");
    // 自動適用で件数の行や「島図を見る」が出ると #fBody の高さが変わるので、
    // 描画が落ち着いてから測る。1回目で足りないことがあるので少し後にもう一度。
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      row.classList.add("just");
      scrollRowIntoBody(row);
    }));
    setTimeout(()=>scrollRowIntoBody(row),420);
    setTimeout(()=>row.classList.remove("just"),2200);
  }
  const n=recNNow(), st=NERAI_REC_STAT[n];
  const tile=document.querySelector('#fNer .ch[data-v="recat"]');
  if(tile&&st){
    const w=tile.querySelector(".ner-w"), s=tile.querySelector(".ner-s");
    if(w) w.textContent="AT機を、直近7日の平均差枚が低い順に並べた上位"+n
      +"台。期間の指定とは関係なく、いつも直近7日で計算します。";
    if(s) s.textContent=st.s;
  }
  const note=document.getElementById("fRecNote");
  if(note) note.textContent="検証したのはこの3つの台数だけです（水曜9日ぶん）。"
    +"1日あたりは 40台 +10,833円 ／ 80台 +10,417円 ／ 150台 +5,883円。"
    +"80台は日ごとのぶれを見てもマイナスに落ちない唯一の台数なので既定にしています。";
  // 「さらに絞る」の欄も同じ条件で出し入れ(2026-09-10)
  const xrow=document.getElementById("fRecXRow");
  if(xrow) xrow.hidden=!on;
  const xnote=document.getElementById("fRecXNote");
  if(xnote) xnote.textContent="上位"+n+"台の中で、全期間の平均差枚が上位半分で、直近7日の最悪日が前日ではない台だけにします"
    +"（だいたい3分の1に減ります）。裏付けは全日86日ぶん: 80台全体のプラス率39%に対して43%、"
    +"前半と後半に分けても両方で上回りました。水曜13日ぶんでも48%→51%。カバネリは別枠なので条件には入れていません。";
}
...
function neraiRecList(){
  ...
  Object.keys(DATA.machines||{}).forEach(d=>{
    ...
    // 「さらに絞る」の材料(2026-09-10)。v[0] が前日。
    //   worstLast … 直近7日の最悪日が前日か(前日が窓の最小値)
    //   mall      … 全期間(その台に記録がある日ぜんぶ)の平均差枚=地力。20日未満は無し
    let mn=v[0]; for(const x of v){ if(x<mn) mn=x; }
    let s=0, c=0;
    (m.d||[]).forEach(x=>{ if(x&&x[0]!=null){ s+=x[0]; c++; } });
    a.push({d:String(d), m:v.reduce((s2,x)=>s2+x,0)/v.length,
            worstLast:(v[0]===mn), mall:(c>=20?s/c:null)});
  });
...
// 「さらに絞る」(2026-09-10・谷川氏指示)。上位n台の中で
//   (1) 地力(全期間の平均差枚)が、その n台の中で上位半分(中央値以上)
//   (2) 直近7日の最悪日が前日ではない
// の両方に当たる台だけに狭める。裏付けは nerai80_rules.py の Z2
//   (全日86日 42.9%・上乗せ+3.6pt・前半+4.4→後半+3.0 ／ 水曜13日 51.0%・前半+5.2→後半+3.0)。
//   ★上位1/3にすると全日ではもう少し強い(43.4%)が水曜の前半で反転したので、
//     再現する「上位半分」を採る。カバネリは別枠(条件に入れない)。
function neraiRecXOn(){
  return !!document.querySelector('#fRecX > .ch.on');
}
function neraiRecRefine(list){
  const has=list.filter(x=>x.mall!=null).map(x=>x.mall).sort((p,q)=>p-q);
  if(!has.length) return [];
  const k=has.length>>1;
  const med=(has.length%2)?has[k]:(has[k-1]+has[k])/2;
  return list.filter(x=>x.mall!=null&&x.mall>=med&&!x.worstLast);
}
function neraiRecSet(n){
  const top=neraiRecList().slice(0,n||NERAI_REC_N);
  return new Set((neraiRecXOn()?neraiRecRefine(top):top).map(x=>x.d));
}
function neraiLabel(){
  ...
  if(on.indexOf("recat")>=0) t.push("直近7日で凹んだAT機 上位"+recNNow()+"台"
    +(neraiRecXOn()?"（さらに絞る: 地力↑＋前日最悪×）":""));
  return "／狙い方: "+t.join("＋");
}
...
(()=>{
  ...
  box.addEventListener("click",e=>{
    ...
    // ★「狙う台数」の欄は #fNer の中に入れ子で置いてある(2026-08-23夕)。
    //   そこのチップ(40/80/150)を押しただけで狙い方が外れないよう、ここでは無視する。
    if(b.closest("#fRecNRow")) return;
    if(b.closest("#fRecXRow")) return;   // 「さらに絞る」も入れ子(2026-09-10)
    ...
    // 引数の true は「押して出したので、その欄まで送ってよい」の合図(2026-08-23)
    syncRecN(true);   // 台数の欄は「直近7日で凹んだAT機」を選んでいるときだけ出す
  });
})();
// 狙う台数(2026-08-23)。期間と同じで**1つだけ**選ぶ=同時に2つの台数は成り立たない。
// ★絞り込みの当て直しは #filterCard の委譲(fAutoApply)が拾うので、ここでは書かない
//   (二重に走ると839台の走査が2回になる)。
(()=>{
  const box=document.getElementById("fRecN");
  if(!box) return;
  box.addEventListener("click",e=>{
    const b=e.target&&e.target.closest?e.target.closest(".ch"):null;
    if(!b||!box.contains(b)||b.disabled) return;
  ...
  syncRecN();   // 読み込み時に文言と既定(80台)をそろえておく
})();
// さらに絞る(2026-09-10)。押すたびに入/切。当て直しは #filterCard の委譲(fAutoApply)が拾う。
(()=>{
  const box=document.getElementById("fRecX");
  if(!box) return;
  box.addEventListener("click",e=>{
    const b=e.target&&e.target.closest?e.target.closest(".ch"):null;
    if(!b||!box.contains(b)||b.disabled) return;
...
document.getElementById("fApply").addEventListener("click",()=>{
  ...
  // 狙い方その2は「上位◯台」という数の指定なので、台を1つずつ見る前に
  // 1回だけ表を作る(839台ぶん作り直すと重い)。台数は画面で選べる(2026-08-23)。
  const recSet=(ner.indexOf("recat")>=0)?neraiRecSet(recNNow()):null;   // 「さらに絞る」込み(2026-09-10)
  ...
  if(!perNow()){
    ...
    // グラフ一覧も一覧と同じ扱い(2026-08-24)
    const gb0=document.getElementById("fGraph");
    if(gb0){ gb0.disabled=true; gb0.textContent="グラフ一覧"; }
    const gx0=document.getElementById("fGraphBox");
    if(gx0){ gx0.hidden=true; gx0.innerHTML=""; }
  ...
  document.querySelectorAll(".tap").forEach(el=>{
    ...
    // 狙い方その2(2026-08-20夜)。直近7日で凹んだAT機の上位(選んだ台数)に入っているか。
  ...
  // グラフ一覧も同じ(2026-08-24)。開いたままなら条件を変えるたびに描き直す。
  const gb=document.getElementById("fGraph");
  if(gb){ gb.disabled=!(active&&hitN); }
  const gx=document.getElementById("fGraphBox");
  if(gx&&!gx.hidden) paintFilterGraphs();
...
// ==== まとめてピン留め(2026-08-24夕・谷川さん指示「機種名の左側に機種毎にピン留め
//      チェックボックス作成と一覧全てをピン留めするチェックボックスを作成」) ====
// 台1つずつ押さなくても、機種まるごと・一覧まるごとを一度に留められるようにする。
// ★入れ物は台番カードの★と同じ(localStorage: shimaheat-pins)。別に持たせると
//   片方で外したのにもう片方に残る、という食い違いが必ず起きる。
// ★上限に当たったら**黙って落とさない**。入ったぶんと入らなかったぶんを言葉で返す。
function pinState(ids){
  const p=new Set(loadPins());
  let on=0;
  ids.forEach(d=>{ if(p.has(String(d))) on++; });
  return !ids.length?"none":(on===0?"none":(on===ids.length?"all":"some"));
}
function pinBulk(ids,on){
  const pins=loadPins();
  if(on){
    const have=new Set(pins.map(String));
    let add=0,over=0;
    ids.forEach(d=>{
      d=String(d);
      if(have.has(d)) return;
      if(pins.length>=PIN_MAX){ over++; return; }
      pins.push(d); have.add(d); add++;
    });
    savePins(pins.slice(0,PIN_MAX));
    showToast(over
      ? (add+"台を留めました。上限"+PIN_MAX+"台に達したので残り"+over+"台は入りません")
      : (add+"台をピン留めしました"), over?3000:1600);
  }else{
    const rm=new Set(ids.map(String));
    const left=pins.filter(d=>!rm.has(String(d)));
    const n=pins.length-left.length;
    savePins(left);
    showToast(n+"台のピンを外しました",1600);
  }
  paintPins();
}
// 見出しに置くチェック。ids は「その見出しが受け持つ台番」。
function pinCkHtml(ids,label){
  const st=pinState(ids);
  return '<label class="pinck" title="'+esc(label)+'">'
    +'<input type="checkbox" class="pinck-cb"'+(st==="all"?" checked":"")
    +' data-ids="'+esc(ids.join(","))+'" aria-label="'+esc(label)+'">'
    +'<span class="pm" aria-hidden="true">★</span></label>';
}
// 一部だけ留まっている見出しは「中間」の見た目にする(チェックでも空でもない)。
// これが無いと、機種の中の1台だけ留めたときに見出しが空に見えて実態と食い違う。
function syncPinCks(){
  const p=new Set(loadPins());
  document.querySelectorAll(".pinck-cb").forEach(cb=>{
    ...
    // ★が留まっているかを色でも出す(2026-08-24夜)。チェックの四角だけだと
    //   隣の「比」のチェックと見分けが付きにくい。
    const lb=cb.closest?cb.closest(".pinck"):null;
    if(lb){ lb.classList.toggle("on",on>0);
            lb.classList.toggle("mid",on>0&&on<ids.length); }
    // ★一覧まるごとのチェックは「いま何台入っているか」を数でも出す
    //   (2026-08-24夜・谷川さん報告「機種のチェックを押したら一覧の80台の方まで
    //    一緒にチェックが入った」。実際に入ったのは機種の14台だけで、一覧は
    //    “一部だけ”の中間表示だったが、塗り方が留まっている状態とそっくりだった)。
    const row=cb.closest?cb.closest(".pinall"):null;
    const nn=row?row.querySelector(".pinn"):null;
    if(nn) nn.textContent=on?("（いま"+on+"台）"):"";
  });
}
(function(){
  ["fListBox","fGraphBox"].forEach(id=>{
    const box=document.getElementById(id); if(!box) return;
    box.addEventListener("change",e=>{
      const cb=e.target&&e.target.classList
        &&e.target.classList.contains("pinck-cb") ? e.target : null;
      if(!cb) return;
      e.stopPropagation();
      const ids=String(cb.dataset.ids||"").split(",").filter(Boolean);
      // ★押したときの見た目ではなく「いまの実態」で決める(2026-08-24夕)。
      //   まだ留められる余地がある → まとめて留める（半分だけ留まっている所も埋める）
      //   全部留まっている／上限で1台も増やせない → まとめて外す
      //   この決め方なら、上限に当たって一部しか入らなかった場合も
      //   もう一度押せば必ず外せる(見た目のcheckedで判断すると外せず詰む。実測で発生)。
      const p=new Set(loadPins().map(String));
      const room=loadPins().length<PIN_MAX;
      const canAdd=room&&ids.some(d=>!p.has(String(d)));
      pinBulk(ids,canAdd);
    });
  });
})();
...
function paintFilterList(){
  ...
  // 一覧まるごとのピン留めで使う台番(2026-08-24夕)
  const allDai=[];
  names.forEach(nm=>{ g[nm].forEach(x=>allDai.push(x.dai)); });
  ...
  let h='<div id="fCkBox"></div>'
        ...
        // 一覧まるごとのピン留め(2026-08-24夕・谷川さん指示)。機種の見出しのチェックと
        // 同じ入れ物を動かすので、押した瞬間に下の機種側のチェックも追従する。
        +'<div class="pinall">'
        +pinCkHtml(allDai,"この一覧の"+allDai.length+"台をまとめてピン留め")
        +'<span>この一覧の'+allDai.length+'台をまとめてピン留め'
        +'<span class="pinn"></span></span></div>'
        // ★2種類のチェックが同居しているので、役割を最初に1回だけ書く
        //   (2026-08-24夜・谷川さん指示「使ってみて紛らわしいようなら整理する」)。
        //   見た目でも分ける: ピン留め＝★と紺 ／ 見比べ＝「比」の列と黄土色。
        +'<div class="fl-leg">'
        +'<span class="lg-pin">★</span>のチェック＝島図にピン留め'
        +'（開き直しても残ります）<br>'
        +'<span class="lg-ck">「比」</span>のチェック＝この場だけの見比べ'
        +'（下の「チェックした台」に集まり、パネルを閉じると消えます）</div>'
        ...
        //   「◯番目」は狙い表を通した順番なので、そこも言葉で分ける。
        //   ★台数は画面で選べる(2026-08-23)ので、ここも選んだ台数を書く。
        +(recOn?('<div class="fl-note">「直近7日」はその台の直近7日の平均差枚。'
                 +'一覧は機種ごとにまとめ、その中を凹みの深い順に並べています'
                 +'（数字の下の「◯番目」は狙い表'+recNNow()+'台を通した順番）。<br>'
  ...
  names.forEach(nm=>{
    ...
    h+='<div class="fl-g" data-nm="'+esc(nm)+'"><div class="fl-gn">'
      // ★機種名の左にピン留めのチェック(2026-08-24夕・谷川さん指示)。
      //   列は増やせない(8列にすると390pxで35pxはみ出す)ので見出しに置く。
      +pinCkHtml(rows.map(x=>x.dai),nm+" の"+rows.length+"台をまとめてピン留め")
      +esc(nm)
      +'<span>'+rows.length+'台'
      ...
      +'<table class="fl-t"><thead><tr>'
      +'<th class="fl-ckth" title="この場で見比べる台に入れる">比</th><th>台番</th>'
      ...
      +rows.map(x=>'<tr>'+ckCell(x.dai)+'<td class="fl-d">'+daiBtn(x.dai)
        ...
        +'</td><td class="fl-r'+cls((x.r==null)?null:(x.r-100))+'">'+fpc(x.r)
        +'</td><td class="fl-a">'+gnum(x.a)
        +'</td><td class="fl-pl">'+rate(x.pl)
        +'</td></tr>').join("")+'</tbody></table></div>';
  });
  box.innerHTML=h;
  syncPinCks();          // 一部だけ留めた見出しを中間の見た目に
  paintCheckedList();
...
function paintCheckedList(){
  ...
  box.innerHTML='<div class="fl-ckh"><span class="fl-ckt">チェックした台（'+rows.length+'台'
    ...
    // ★ピン留めとの違いをここでも1行だけ言う(2026-08-24夜)。
    +'<div class="fl-ckn">この場だけの選び分けです（画面を開き直すと消えます）。'
    +'ずっと残したいときは★のピン留めを使ってください</div>'
    +'<table class="fl-t fl-tc"><thead><tr>'
    +'<th class="fl-ckth" title="この場で見比べる台に入れる">比</th>'
    +'<th>台番</th><th>機種</th>'
    +'<th>差枚</th><th>出率</th><th>平均G</th><th>ﾌﾟﾗｽ率</th></tr></thead><tbody>'
    +rows.map(x=>'<tr>'+ckCell(x.dai)+'<td class="fl-d">'+daiBtn(x.dai)
...
// 一覧を開く前に見ていた場所(2026-08-24・谷川さん報告「一覧閉じたときは元の画面位置で」)。
// 一覧は長い(実測で中身が2744→7636pxに伸びる)ので、開くと下まで送られる。閉じると
// 中身が元の長さへ戻るぶん、送った位置がスクロールの上限に丸められて**最下部に張り付く**
// (実測: 0 → 開いて2744 → 閉じて2230=上限)。開く直前の位置を覚えて戻す。
//
// ★戻り先が「いまの中身では届かない位置」のときは先頭へ戻す(2026-08-24夕)。
//   谷川さん報告「台一覧を閉じるとこの画面(空白)になってしまう」。順番を変えて
//   総当たりで測ると、グラフ一覧を開いた状態で台一覧を開き→グラフを閉じ→台一覧を
//   閉じる、という順で戻り先(2744)が今の上限(2230)を超える。上限へ丸めると
//   パネルのいちばん下＝ほぼ何も無い所に貼り付き、実機では空白に見える。
//   届かないときは素直に先頭から見せる方が、探し直しが要らない。
function fBodyBack(top){
  const b=document.getElementById("fBody"); if(!b) return;
  const put=()=>{
    const max=Math.max(0,b.scrollHeight-b.clientHeight);
    b.scrollTop=(top>max)?0:top;
  };
  put();
  requestAnimationFrame(put);
  setTimeout(put,140);        // 中身が縮み切った後にもう一度当てる
}
let fListBackTop=0;
document.getElementById("fList").addEventListener("click",()=>{
  const box=document.getElementById("fListBox"); if(!box) return;
  const body=document.getElementById("fBody");
  if(box.hidden){ fListBackTop=body?body.scrollTop:0;
    paintFilterList(); box.hidden=false;
    document.getElementById("fList").textContent="一覧を閉じる";
    box.scrollIntoView({block:"nearest",behavior:"smooth"}); }
  else { box.hidden=true; box.innerHTML="";
    document.getElementById("fList").textContent="絞り込み台一覧";
    // 中身が縮んだ後に戻す(届かない位置なら先頭へ)。
    fBodyBack(fListBackTop); }
});
// ---- グラフ一覧(2026-08-24新設・谷川さん指示「クリアボタンをグラフ一覧という
//      ボタンに変更」) ----
// 「絞り込み台一覧」が数字の表なのに対して、こちらは**当たった台の形**を並べる。
// 1台ずつカードを開かなくても、伸びている台・沈んでいる台のかたちを一度に見比べられる。
// ★線は「選んだ期間の中の累計差枚」＝台番カードのグラフと同じ読み方
//   (1日ぶんの棒ではない)。日を狭めているときは狭めた日だけを順につないだ形になる。
// ★0の線を必ず入れる(上下だけでは、伸びているのか沈んでいるのかが読めないため)。
function fgSpark(v,W,H){
  if(!v.length) return "";
  let mn=v[0],mx=v[0];
  for(const q of v){ if(q<mn)mn=q; if(q>mx)mx=q; }
  const lo=Math.min(0,mn), hi=Math.max(0,mx), sp=(hi-lo)||1;
  const y=q=>H-2-((q-lo)/sp)*(H-4);
  const x=i=>(v.length<2?W/2:(i/(v.length-1))*W);
  let d="";
  v.forEach((q,i)=>{ d+=(i?"L":"M")+x(i).toFixed(1)+" "+y(q).toFixed(1)+" "; });
  const last=v[v.length-1];
  const cls=last>0?"fg-up":(last<0?"fg-dn":"");
  const z=y(0).toFixed(1);
  return '<svg class="fg-s '+cls+'" viewBox="0 0 '+W+' '+H+'" '
        +'preserveAspectRatio="none" aria-hidden="true">'
        +'<line class="fg-z" x1="0" y1="'+z+'" x2="'+W+'" y2="'+z
        +'" vector-effect="non-scaling-stroke"/>'
        +'<path class="fg-l" d="'+d.trim()+'" fill="none" '
        +'vector-effect="non-scaling-stroke"/></svg>';
}
// 並び順(2026-08-24夜・谷川さん指示「グラフ一覧の並び順・出す情報を見直す」)。
// ★既定は「機種ごと」。どの機種に何台当たったかを数えながら見るための並び。
// ★「差枚が多い順／少ない順」は機種をまたいで端から見るための並び。区切りを外して
//   1つのまとまりにし、代わりにカードへ機種名を小さく出す(でないと、どの機種の台か
//   分からなくなる)。
// ★選んだ並びは覚えておく(次に開いたときも同じ)。
const FG_SORTS=[["kishu","機種ごと"],["high","差枚が多い順"],["low","差枚が少ない順"]];
function fgSortNow(){
  let v=null; try{ v=localStorage.getItem("shimaheat-fgsort"); }catch(e){}
  return FG_SORTS.some(s=>s[0]===v)?v:"kishu";
}
// 出す場所(2026-08-24夜・谷川さん指示「グラフ一覧おしたらグラフ一覧を狙う台数の
// 下に表示させる」)。「狙う台数」が出ているとき＝狙い方「直近7日で凹んだAT機」を
// 押しているときは、その真下へ移す(2026-08-23夕に「狙う台数」を押したカードの
// 真下へ移したのと同じ並べ方。どのカードに効いている一覧なのかを近さで示す)。
// 出ていないときは元の場所(#fGraphHome の直後)へ戻す。
// ★入れ子にするので、#fNer の中の受け口が拾わないことを必ず確かめる
//   (2026-08-23夕に「狙う台数」で実際に踏んだ。汎用の複数選択ハンドラが
//    入れ子のボタンまで拾って .on を打ち消した)。
// 2026-08-24夕・谷川さん指示「台一覧押した時はを狙う台数の下に表示する」。
// グラフ一覧と同じ扱いにしたので、置き場所を決める所を1つにまとめた。
// ★両方開いたときの並びは「グラフ一覧 → 台一覧」で固定する
//   (開いた順で入れ替わると、同じ操作をしても場所が変わって探しにくい)。
const FBOX_HOME={fGraphBox:"fGraphHome", fListBox:"fListHome"};
function fPlaceBox(id){
  const box=document.getElementById(id); if(!box) return;
  const rec=document.getElementById("fRecNRow");
  const home=document.getElementById(FBOX_HOME[id]);
  if(rec&&!rec.hidden){
    let anchor=rec;
    if(id==="fListBox"){
      const g=document.getElementById("fGraphBox");
      // グラフ一覧が開いていて同じ入れ子に居るときは、その後ろへ
      if(g&&!g.hidden&&g.closest("#fNer")) anchor=g;
    }
    if(box.previousElementSibling!==anchor) anchor.after(box);
    box.classList.add("innr");
  }else if(home){
    if(box.previousElementSibling!==home) home.after(box);
    box.classList.remove("innr");
  }
}
function fgPlace(){ fPlaceBox("fGraphBox"); }
function paintFilterGraphs(){
  const box=document.getElementById("fGraphBox"); if(!box) return;
  fgPlace();
  const per=perNow(), idx=mIdx(per);
  const sortBy=fgSortNow();
  const items=[];
  document.querySelectorAll(".tap.filt-hit").forEach(el=>{
    const dai=el.dataset.dai, m=DATA.machines[dai]||{};
    let s=0; const v=[];
    idx.forEach(i=>{ const x=(m.d||[])[i]; if(x&&x[0]!=null){ s+=x[0]; v.push(s); } });
    items.push({dai:dai, nm:m.n||"（機種名なし）", v:v,
                last:v.length?v[v.length-1]:null, r:mRate(dai,per)});
  });
  if(!items.length){ box.innerHTML='<div class="fl-none">該当した台がありません</div>'; return; }
  const num=x=>(typeof x==="number")?((x>0?"+":"")+x.toLocaleString()):"−";
  const rate=x=>(typeof x==="number")?(x.toFixed(1)+"%"):"−";
  // ピン留め(2026-08-24・谷川さん指示「グラフ一覧の時もピン留めできるように
  // チェックボックス台毎につくる」)。**台番カードの★と同じ入れ物**を使うので、
  // ここで留めた台は島図の印にも「★一覧」にも即その場で出る。
  const pinned=new Set(loadPins());
  const allDai=items.map(x=>x.dai);
  // 1枚のカード。出す数字は「期間の中の最終差枚」と「出率」
  // (2026-08-24夜に出率を追加。差枚だけだと、よく回っている台ほど大きく出るので、
  //  機種をまたいで見比べるときの物差しが無かった)。
  const card=(it,withName)=>{
    const on=pinned.has(String(it.dai));
    return '<div class="fg-c'+(on?" on":"")+'" data-dai="'+esc(it.dai)+'">'
      +'<div class="fg-t">'
      +'<label class="fg-p"><input type="checkbox" class="fg-cb" data-dai="'
      +esc(it.dai)+'"'+(on?" checked":"")+' aria-label="台'+esc(it.dai)
      +'をピン留めする"><span class="fg-pm" aria-hidden="true">★</span></label>'
      +'<b>'+esc(it.dai)+'</b><em class="'
      +(it.last>0?"fl-p":(it.last<0?"fl-m":""))+'">'+num(it.last)+'</em></div>'
      +'<div class="fg-t2">'
      +(withName?('<span class="fg-nm">'+esc(it.nm)+'</span>'):"")
      +'<span class="fg-rt'+((it.r==null)?"":(it.r>=100?" fl-p":" fl-m"))
      +'">出率 '+fpc(it.r)+'</span></div>'
      +'<button type="button" class="fg-b fl-go" data-dai="'+esc(it.dai)
      +'" aria-label="台'+esc(it.dai)+'のカードを開く">'
      +fgSpark(it.v,120,42)+'</button></div>';
  };
  let h='<div class="fg-hp">期間: '+esc(perLabel())+' ／ 該当 '+items.length+'台'
       +'<em>線は期間の中の累計差枚。横の線が0枚です。グラフを押すとその台のカードが'
       +'開きます。左の★のチェックでその台を島図にピン留めできます（'+PIN_MAX
       +'台まで）。</em></div>'
       +'<div class="fg-sortl">並び順</div>'
       +'<div class="fg-sort" role="group" aria-label="グラフの並び順">'
       +FG_SORTS.map(s=>'<button type="button" class="fgs'+(s[0]===sortBy?" on":"")
         +'" data-s="'+s[0]+'" aria-pressed="'+(s[0]===sortBy?"true":"false")+'">'
         +s[1]+'</button>').join("")+'</div>'
       // 一覧まるごとのピン留め(2026-08-24夕・谷川さん指示)
       +'<div class="pinall">'
       +pinCkHtml(allDai,"この一覧の"+allDai.length+"台をまとめてピン留め")
       +'<span>この一覧の'+allDai.length+'台をまとめてピン留め'
        +'<span class="pinn"></span></span></div>';
  if(sortBy==="kishu"){
    const g={};
    items.forEach(it=>{ (g[it.nm]=g[it.nm]||[]).push(it); });
    const names=Object.keys(g);
    // 並びは台一覧と同じ(台数の多い機種から)。見比べる順番が2つの画面で食い違わないように。
    names.sort((a,b)=>(g[b].length-g[a].length)||a.localeCompare(b,"ja"));
    names.forEach(nm=>{
      const a=g[nm].slice().sort((x,y)=>String(x.dai).localeCompare(
        String(y.dai),"ja",{numeric:true}));
      // ★機種名の左にピン留めのチェック(2026-08-24夕・谷川さん指示)
      h+='<div class="fg-g"><div class="fg-gh">'
        +pinCkHtml(a.map(x=>x.dai),nm+" の"+a.length+"台をまとめてピン留め")
        +esc(nm)+'<span>'+a.length+'台</span></div><div class="fg-grid">'
        +a.map(it=>card(it,false)).join("")+'</div></div>';
    });
  }else{
    // 差枚順は機種をまたぐので、区切りを作らず1つのまとまりに並べる。
    // ★数字の無い台(その期間に実績が無い)は末尾へ回す。並びの端に混ざると
    //   「いちばん沈んだ台」と読み違える。
    const sgn=(sortBy==="high")?-1:1;
    const a=items.slice().sort((x,y)=>{
      const xv=(typeof x.last==="number"), yv=(typeof y.last==="number");
      if(xv!==yv) return xv?-1:1;
      if(!xv) return String(x.dai).localeCompare(String(y.dai),"ja",{numeric:true});
      return sgn*(x.last-y.last);
    });
    h+='<div class="fg-g"><div class="fg-grid">'
      +a.map(it=>card(it,true)).join("")+'</div></div>';
  }
  box.innerHTML=h;
  syncPinCks();          // 一部だけ留めた見出しを中間の見た目にする
}
(function(){
  const box=document.getElementById("fGraphBox"); if(!box) return;
  box.addEventListener("click",e=>{
    // 並び順(2026-08-24夜)。押した位置をなるべく保つ(中身の高さが変わるので丸める)。
    const sb=e.target&&e.target.closest?e.target.closest(".fgs"):null;
    if(sb){
      e.stopPropagation();
      const body=document.getElementById("fBody");
      const keep=body?body.scrollTop:0;
      try{ localStorage.setItem("shimaheat-fgsort",sb.dataset.s||"kishu"); }catch(err){}
      paintFilterGraphs();
      if(body) body.scrollTop=Math.min(keep,
        Math.max(0,body.scrollHeight-body.clientHeight));
      return;
    }
    const go=e.target&&e.target.closest?e.target.closest(".fl-go"):null;
    if(go){ e.stopPropagation(); openDaiFromList(go.dataset.dai); }
  });
  // チェック＝ピン留め(2026-08-24)。台番カードの★と同じ入れ物を書き換えるので、
  // 島図の印・「★一覧」・検索パネルのピン一覧が同時に追従する。
  // ★上限(40台)に当たったら**黙って落とさない**。戻して理由を出す。
  box.addEventListener("change",e=>{
    const cb=e.target&&e.target.classList&&e.target.classList.contains("fg-cb")
      ? e.target : null;
    if(!cb) return;
    const d=String(cb.dataset.dai), pins=loadPins(), i=pins.indexOf(d);
    if(cb.checked){
      if(i<0){
        if(pins.length>=PIN_MAX){
          cb.checked=false;
          showToast("ピンは"+PIN_MAX+"台までです。どれかを外してください",2400);
          return;
        }
        pins.unshift(d);
        showToast("台"+d+"をピン留めしました",1400);
      }
    }else if(i>=0){
      pins.splice(i,1);
      showToast("台"+d+"のピンを外しました",1400);
    }
    savePins(pins.slice(0,PIN_MAX));
    paintPins();
  });
})();
// 開け閉ての作法は一覧とそろえる(開く前の場所を覚えて、閉じたら戻す)。
let fGraphBackTop=0;
document.getElementById("fGraph").addEventListener("click",()=>{
  const box=document.getElementById("fGraphBox"); if(!box) return;
  const b=document.getElementById("fGraph");
  const body=document.getElementById("fBody");
  if(box.hidden){ fGraphBackTop=body?body.scrollTop:0;
    paintFilterGraphs(); box.hidden=false;
    b.textContent="グラフを閉じる";
    box.scrollIntoView({block:"nearest",behavior:"smooth"}); }
  else { box.hidden=true; box.innerHTML="";
    b.textContent="グラフ一覧";
    fBodyBack(fGraphBackTop); }
});
...
function repIntent(cur){
  ...
  if(cur.ner&&cur.ner.indexOf("recat")>=0) a.push("直近7日で凹んでいるAT機"
    +(neraiRecXOn()?"（その中でも地力が高く、前日が最悪日ではない台）":""));
...
function paintFilterReport(){
  ...
  h+='<div class="fr-s">① この絞り込みは全体とどう違うか</div>'
    ...
    +'<tr><td class="fr-k">出率</td><td class="'+cl(now.rate==null?null:now.rate-100)+'">'
      +fpc(now.rate)+'</td><td class="'+cl(base.rate==null?null:base.rate-100)+'">'
      +fpc(base.rate)+'</td><td class="'+cl(d2)+'">'+pt(d2)+'</td></tr>'
  ...
  h+='<div class="fr-s">⑤ おすすめの台番</div>'
    ...
    +dlist("安定して出ている順（稼働3日以上）",byS,
           x=>'<td>'+pv(x.plus)+' ／ '+fpc(x.r)+'</td>',"ﾌﾟﾗｽ率 ／ 出率")
...
document.getElementById("fClear").addEventListener("click",()=>{
  ...
  // ★#fNer は直下だけ。中に入れ子で置いた「狙う台数」は下の setRecN が既定へ戻す
  document.querySelectorAll("#fSue .ch,#fPos .ch,#fDow .ch,#fToku .ch,#fSet .ch,"
                            +"#fNer > .ch")
    .forEach(b=>b.classList.remove("on"));
  setRecN(NERAI_REC_N); syncRecN();   // 狙う台数も既定(80台)へ戻し、欄を引っ込める
  ...
  // グラフ一覧も同じ(2026-08-24)
  const gx=document.getElementById("fGraphBox");
  if(gx){ gx.hidden=true; gx.innerHTML=""; }
  const gb=document.getElementById("fGraph");
  if(gb){ gb.disabled=true; gb.textContent="グラフ一覧"; }
  syncUrl();
});
// ---- 検索(2026-08-01・第2段階) ----
...
/* ==== カードを左右に払って前後の台番へ(2026-09-02・谷川さん指示
   「シマヒートのこの画面で右左にフリックしたら前後の台番のカードに移動して
   みていけるようにして」) ====
   ★動く先は**台番の数字の順**。この店は島に沿って番号が振られているので、
     数字の隣＝だいたい物理的な隣になる。絞り込みの有無では変えない
     （どの入口から開いても同じ動きにする）。
   ★いま見ている期間(curWin)はそのまま引き継ぐ＝直近7日を見ていたら隣も直近7日。
   ★つまみ(#grip/#gripExt)から始まった指は縦に閉じる操作なので触らない。 */
function daiListSorted(){
  const M=(typeof DATA!=="undefined"&&DATA&&DATA.machines)||{};
  return Object.keys(M).filter(d=>/^\d+$/.test(d)).sort((a,b)=>(+a)-(+b));
}
function 隣の台(dir){
  if(!curDai) return null;
  const L=daiListSorted(), i=L.indexOf(String(curDai));
  if(i<0) return null;
  const j=i+dir;
  return (j<0||j>=L.length)?null:L[j];
}
(function(){
  const card=document.getElementById("card"); if(!card) return;
  const grip=document.getElementById("cardGrip"), ext=document.getElementById("gripExt");
  let x0=0,y0=0,t0=0,live=false;
  // 中身が横に動かせる場所(横スクロールする表など)から始まった指は、その場の
  // 操作のほうを優先する。ここを見ないと表を横に送ろうとして台が変わる。
  const 横に動かせる=el=>{
  ...
  // 指についてくる動き(2026-09-02・谷川さん指示「押している間はついてくる感じの仕様に」)。
  // ★transform は positionOverlayCard() も書くが、あちらは cardHeld()(=dragging か
  //   swanim)の間は触らない作りにしてある。だから追従中と送りの動きの最中は
  //   どちらかの class を必ず付けておく。
  // ★素の形は positionOverlayCard と同じ式にする(違う式にすると指を離した瞬間に跳ねる)。
  const 素の形=()=>{
    const vv=window.visualViewport;
    return (vv&&vv.scale)?("scale("+(1/vv.scale)+")"):"";
  };
  const 置く=dx=>{
    const vv=window.visualViewport, s=(vv&&vv.scale)?vv.scale:1;
    card.style.transform="translateX("+(dx*s).toFixed(1)+"px) scale("+(1/s)+")";
  };
  const 戻す=()=>{
    card.classList.remove("dragging"); card.classList.remove("swanim");
    card.style.transition=""; card.style.transform=素の形();
  };
  // 送りの動き(2026-09-02・谷川さん指示「切り替わるカードが左右から出てくる感じに」)。
  //   1) いまの指の位置から、払った向きの画面の外へ滑り出す
  //   2) 外に居る間に中身を入れ替え、反対側の外へ一瞬で置き直す(transitionを切って)
  //   3) そこから中央へ入ってくる
  // ★1と3の間は必ず swanim を付けておく(cardHeld)。付けないと fitCard の描き直しで
  //   transform を中央へ戻され、動きが途中で切れる。
  let 送り中=null;
  const 送る=(d,muki)=>{
    if(送り中){ clearTimeout(送り中); 送り中=null; }
    // 画面の外までの距離。実際に見えている幅で測る(拡大していても正しく外へ出す)。
    const W=card.getBoundingClientRect().width||window.innerWidth;
    const 外=muki*(W+30);
    card.classList.remove("dragging");
    card.classList.add("swanim");
    card.style.transition="transform .16s ease-in";
    置く(外);
    送り中=setTimeout(()=>{
      // 外に出ている間にカードを閉じられていたら、開き直さない
      const md=document.getElementById("modal");
      if(!md||md.style.display!=="block"){
        card.classList.remove("dragging"); card.classList.remove("swanim");
        card.style.transition=""; card.style.transform=素の形(); 送り中=null; return;
      }
      card.classList.add("dragging");     // ここだけ transition を切る(CSSの.dragging)
      card.style.transition="";
      renderCard(d,curWin);
      置く(-外);                           // 反対側の外へ置き直す
      requestAnimationFrame(()=>{
        card.classList.remove("dragging");
        card.style.transition="transform .22s cubic-bezier(.2,.8,.25,1)";
        card.style.transform=素の形();
        送り中=setTimeout(()=>{
          card.classList.remove("swanim"); card.style.transition=""; 送り中=null;
        },240);
      });
    },165);
  };
  ...
  card.addEventListener("touchstart",e=>{
    ...
    if(送り中) return;                                  // 送りの動きの最中は受け付けない
    if(!e.touches||e.touches.length!==1) return;       // 2本指は拡大なので触らない
    const t=e.touches[0], tg=e.target;
    if(grip&&grip.contains(tg)) return;                // つまみは縦に閉じる操作
    if(ext&&ext.contains(tg)) return;
    if(横に動かせる(tg)) return;
    // 画面の左右の端から始まる指は「戻る」に使われるので避ける
    if(t.clientX<24||t.clientX>window.innerWidth-24) return;
    x0=t.clientX; y0=t.clientY; t0=Date.now(); live=true;
  },{passive:true});
  card.addEventListener("touchmove",e=>{
    if(!live) return;
    if(e.touches&&e.touches.length>1){                 // 途中で2本指になったらやめる
      live=false; if(lock){ lock=false; 戻す(); }
      return;
    }
    const t=e.touches[0], dx=t.clientX-x0, dy=t.clientY-y0;
    if(!lock){
      if(Math.abs(dx)<10) return;                      // まだ向きが決まらない
      if(Math.abs(dx)<Math.abs(dy)*1.2){ live=false; return; } // 縦の操作へ譲る
      lock=true; card.classList.add("dragging");       // ここから横に握る
    }
    // 端では引いても3割しか動かさない=これ以上進めないことが指で分かる
    置く(隣の台(dx<0?1:-1)?dx:dx*0.3);
    e.preventDefault();          // 追従中は中身のスクロールを止める
  },{passive:false});
  ...
  card.addEventListener("touchend",e=>{
    ...
    const だった=lock; lock=false;
    const t=e.changedTouches&&e.changedTouches[0];
    if(!t){ if(だった) 戻す(); return; }
    const dx=t.clientX-x0, dy2=t.clientY-y0, dt=Date.now()-t0;
    // 十分に引いたか、短くさっと払ったかのどちらかで切り替える
    const 十分=Math.abs(dx)>=60, 速い=(Math.abs(dx)>=45&&dt<250);
    const 横=Math.abs(dx)>=Math.abs(dy2)*1.5;
    const d=(だった&&(十分||速い)&&横&&dt<=800)?隣の台(dx<0?1:-1):null;  // 左へ払う=次へ
    if(!d){
      if(だった) 戻す();
      if(だった&&(十分||速い)&&横){
        try{ showToast(dx<0?"いちばん大きい台番です":"いちばん小さい台番です",1600); }catch(err){}
      }
      return;
    }
    // 払った向きへ抜けていき、隣のカードが反対側から入ってくる。
    送る(d, dx<0?-1:1);
  },{passive:true});
})();
restoreFromUrl();
...
// 覚えておける上限。ここを1か所にしておく(2026-08-24。グラフ一覧のチェックからも
// 留められるようになり、上限に当たる場面が現実に出てきたため)。
// ★2026-08-24夕に40→200へ引き上げた(谷川さん指示「一覧全てをピン留めする
//   チェックボックスを作成」)。狙い方の最大が150台なので、一覧まるごと留めても
//   入り切る数にしてある。40のままだと「全部」が半分しか入らず、機能として成立しない。
const PIN_MAX=200;
function loadPins(){
...
function paintPins(){
  ...
  // グラフ一覧のチェックもここでそろえる(2026-08-24)。カードの★や「すべて外す」で
  // 動かしたときに、開いたままのグラフ一覧が古い状態で取り残されないように。
  const pset=new Set(pins.map(String));
  document.querySelectorAll("#fGraphBox .fg-cb").forEach(cb=>{
    const on=pset.has(String(cb.dataset.dai));
    cb.checked=on;
    const c=cb.closest(".fg-c");
    if(c) c.classList.toggle("on",on);
  });
  // 機種の見出し・一覧全体のチェックもここでそろえる(2026-08-24夕)
  if(typeof syncPinCks==="function") syncPinCks();
...
/* ==== 島図のパチンコ島の跡へ「機種別まとめ」を置く(2026-08-28・谷川氏指示) ====
   「画像で作ったのと同じ表を、アプリでもパチンコのところに出したい」。
   資料の中にも同じ数字はあるが、島を見ながらその場で確かめられるようにする。
   盤面(#board)の中へ絶対配置で入れるので、拡大縮小・移動は島図と一緒に動く。
   ★セル([data-k])には一切触らない。期間を切り替えて戻したときに style.cssText を
     1文字ずつ比べる検証(verify_shimaheat_url)があるため、新しい要素を足すだけにする。 */
function パチンコの空き(){
  const bd=document.getElementById("board"); if(!bd) return null;
  // 島図本体の実寸で領域を絞る(下の資料テーブルまで含めた高さで割ると位置がずれる)。
  const W=(typeof IW==="number"&&IW)?IW:bd.offsetWidth;
  const H=(typeof IH==="number"&&IH)?IH:bd.offsetHeight;
  let best=null,area=0;
  bd.querySelectorAll(".cell").forEach(el=>{
    if(el.dataset&&el.dataset.dai) return;          // 台のセルは器ではない
    if(el.classList.contains("tc")) return;         // 下の資料の表は対象外
    if((el.textContent||"").trim()) return;         // 文字のあるセルも器ではない
    const x=el.offsetLeft,y=el.offsetTop,w=el.offsetWidth,h=el.offsetHeight;
    if(!w||!h) return;
    if(x<W*0.30||y<H*0.40) return;                  // 右下だけを見る
    if(w*h>area){ area=w*h; best={x:x,y:y,w:w,h:h}; }
  });
  return best;
}
/* 並び替えの状態(2026-08-31・谷川さん指示「項目毎のボタンを上に出して押すと
   表の並び替えができるように」)。d=-1が大きい順、d=1が小さい順。
   ★期間を切り替えても選んだ項目は保つ(描き直しのたびに戻ると押し直しが要る)。 */
let brSort={k:"台平均",d:-1};
function 機種別まとめを盤面へ(){
  const bd=document.getElementById("board"); if(!bd) return;
  let box=document.getElementById("boardRates");
  const R=(typeof ratesDocFor==="function")?ratesDocFor():null;
  const aki=パチンコの空き();
  if(!R||!R.rows||!R.rows.length||!aki){ if(box) box.remove(); return; }
  const c=R.cols||[];
  const IX=(n,d)=>{const i=c.indexOf(n); return i<0?d:i;};
  const iCnt=IX("台数",1), iRate=IX("出率",2);
  const iWin=c.indexOf("勝率");
  // 総G(2026-08-31・谷川さん指示「総Gの項目を平均Gの右側に入れよう」)。
  // 同じ日に「プラス」の列は外した(谷川さん「単日はわかりやすいが他の期間は分かりにくい」。
  // あれは台数ではなくプラスだった台日なので、期間が延びるほど読み取りにくかった)。
  const iTot=c.indexOf("総G");
  const iG=(c.indexOf("平均G")>=0?c.indexOf("平均G"):IX("平均G/台日",4));
  const iAvg=(c.indexOf("差枚/台")>=0?c.indexOf("差枚/台"):IX("平均差枚/台日",5));
  const num=s=>{const v=parseFloat(String(s==null?"":s).replace(/[^0-9.+-]/g,""));
                return isFinite(v)?v:-1e12;};
  // 並び替え。既定は画像と同じ「台平均の高い順」で、上のボタンで替えられる。
  // ★総差枚だけは文字ではなく数値(r.sv)を持っているので -2 を目印にして別扱いにする。
  const 項目=[["機種",0],["設置台数",iCnt],["勝率",iWin],
              ["平均G",iG],["総G",iTot],["出率",iRate],["台平均",iAvg],["総差枚",-2]]
             .filter(x=>x[1]>=0||x[1]===-2);
  if(!項目.some(x=>x[0]===brSort.k)) brSort={k:"台平均",d:-1};
  const 並び値=(r,i)=>(i===-2)
      ?((typeof r.sv==="number")?r.sv:-1e12)
      :num((r.v||[])[i]);
  const kIdx=(項目.find(x=>x[0]===brSort.k)||["台平均",iAvg])[1];
  const rows=(R.rows||[]).slice().sort((a,b)=>{
    if(brSort.k==="機種"){
      // 機種名は数字にならないので五十音順で比べる
      return String((a.v||[])[0]||"").localeCompare(String((b.v||[])[0]||""),"ja")*brSort.d;
    }
    return (並び値(a,kIdx)-並び値(b,kIdx))*brSort.d;
  });
  const 列=3, 行数=Math.ceil(rows.length/列);
  const cell=(t,cls)=>'<td'+(cls?' class="'+cls+'"':'')+'>'+esc(t==null?"":t)+'</td>';
  // 総Gを万・億でまとめる(2026-08-31・谷川さん指示「総G数が数字桁数が多く視認性が
  // 悪いため万や億をつかおう」)。28,374,528 の10字が 2,837万 の5字になる。
  //   1万未満        … そのまま(2,171)
  //   1万〜100万未満  … 小数1桁(24.7万)。桁が減っても大小の差が読める
  //   100万〜1億未満  … 小数なし(2,837万)
  //   1億以上        … 小数2桁まで(1.23億)。末尾の0は落とす
  // ★並び替えは生の数(v[iTot])で行うのでこの丸めの影響を受けない。
  const 万億=x=>{
    const n=num(x);
    if(n<=-1e11) return "−";
    const a=Math.abs(n), s=(n<0?"-":"");
    if(a>=1e8) return s+(a/1e8).toFixed(2).replace(/\.?0+$/,"")+"億";
    if(a>=1e6) return s+Math.round(a/1e4).toLocaleString()+"万";
    if(a>=1e4) return s+(a/1e4).toFixed(1)+"万";
    return s+Math.round(a).toLocaleString();
  };
  let html="";
  for(let g=0;g<列;g++){
    // ★列幅は中身の要る幅で配る(2026-08-28)。等分だと総差枚(-638,901)と出率が
    //   はみ出して隣の組の機種名に重なる。機種名を略名にしたので機種の列は狭くできる。
    html+='<div class="brg"><table>'
      // 2026-08-31: プラスを外し、平均Gの右へ総Gを入れた。見出しは「台」→「設置台数」。
      // 幅は中身の実幅(全角2/半角1)の見積りで配る。総Gを万・億にしたので
      // 「2,837万」の7で足り、16→12%へ戻した(残りを機種と総差枚へ回す)。
      // 設置台数は見出し(8字ぶん)のほうが中身より広い。
      +'<colgroup>'+[24,13,7,9,12,10,10,15].map(w=>'<col style="width:'+w+'%">').join('')
      +'</colgroup><thead><tr>'
      +'<th class="bn">機種</th><th>設置台数</th><th>勝率</th>'
      +'<th>平均G</th><th>総G</th><th>出率</th><th>台平均</th><th>総差枚</th>'
      +'</tr></thead><tbody>';
    for(let i=g*行数;i<Math.min((g+1)*行数,rows.length);i++){
      const r=rows[i], v=r.v||[];
      const av=num(v[iAvg]), rt=num(v[iRate]);
      const sv=(typeof r.sv==="number")
        ?'<td class="'+(r.sv>0?"br-p":(r.sv<0?"br-m":""))+'">'
          +esc((r.sv>0?"+":"")+r.sv.toLocaleString())+'</td>'
        :'<td>−</td>';
      html+='<tr>'+cell(v[0],"bn")+cell(v[iCnt])
        +cell(iWin>=0?v[iWin]:"−")
        +cell(v[iG])+cell(iTot>=0?万億(v[iTot]):"−")
        +cell(v[iRate], rt>=100?"br-p":(rt>-1e11?"br-m":""))
        +cell(v[iAvg], av>=0?"br-p":(av>-1e11?"br-m":""))
        +sv+'</tr>';
    }
    html+='</tbody></table></div>';
  }
  if(!box){
    box=document.createElement("div"); box.id="boardRates"; bd.appendChild(box);
    // ★盤面は指でつまんで動かせるので、ボタンを押した指が島図の移動に化けないよう
    //   下ごしらえの段階(capture)で止める。押した後は自分で描き直す。
    ["pointerdown","touchstart","mousedown"].forEach(t=>box.addEventListener(t,e=>{
      if(e.target&&e.target.closest&&e.target.closest(".brs button")) e.stopPropagation();
    },true));
    box.addEventListener("click",e=>{
      const b=(e.target&&e.target.closest)?e.target.closest(".brs button"):null;
      if(!b) return;
      e.preventDefault(); e.stopPropagation();
      const k=b.dataset.brk;
      // 同じ項目をもう一度押したら向きを入れ替える。別の項目なら、数字は大きい順から、
      // 機種名は五十音順から始める(そのほうが最初の一押しで見たい形になる)。
      if(brSort.k===k) brSort.d=-brSort.d;
      else { brSort.k=k; brSort.d=(k==="機種")?1:-1; }
      try{ 機種別まとめを盤面へ(); }catch(err){}
    });
  }
  // ★島の罫線に乗せない(2026-08-28・谷川氏「罫線がガタガタになってる」)。
  //  空きセルの縁がそのまま島の線なので、ぴったり置くと1行おきの帯が線を塗り
  //   つぶし、線が破線のように見える。器の大きさから比で内側へ寄せる
  //   (決め打ちのpxだと拡大率で効きが変わる)。
  const mx=Math.max(2,aki.w*0.010), my=Math.max(2,aki.h*0.018);
  box.style.left=(aki.x+mx)+"px"; box.style.top=(aki.y+my)+"px";
  box.style.width=(aki.w-mx*2)+"px"; box.style.height=(aki.h-my*2)+"px";
  // 字の大きさは器から決める。決め打ちにすると期間で行数が変わったときにはみ出す。
  const fs=Math.max(4,Math.min(aki.h/(行数+5)*0.74, aki.w/列*0.052));
  box.style.fontSize=fs.toFixed(2)+"px";
  const nm=(typeof periodNameNow==="function")?periodNameNow():"";
  // 並び替えのボタン。押している項目に印(▼=大きい順 / ▲=小さい順)を付ける。
  const 印=k=>(brSort.k===k?(brSort.d<0?" ▼":" ▲"):"");
  const btns='<div class="brs">'+項目.map(x=>
      '<button type="button" data-brk="'+esc(x[0])+'"'
      +(brSort.k===x[0]?' class="on"':'')+'>'+esc(x[0]+印(x[0]))+'</button>').join("")
    +'</div>';
  box.innerHTML='<div class="brt">機種別まとめ'+(nm?"【"+esc(nm)+"】":"")
    +'<span class="brn">※ 出率、台平均、総差枚はざっくりの計算のためあくまで目安となります</span>'
    +'</div>'+btns+'<div class="brw">'+html+'</div>';
  // ★器に収まるまで字を縮める。行数は期間で変わり、数字の桁も日で変わるので、
  //   決め打ちの大きさだと必ずどこかではみ出す(最初これで横へあふれた)。
  //   いったん表を自然な幅で組ませて実測し、その比で字を決める。
  const tabs=[...box.querySelectorAll(".brg table")];
  tabs.forEach(t=>{ t.style.tableLayout="auto"; t.style.width="auto"; });
  let 比=1;
  box.querySelectorAll(".brg").forEach(g=>{
    const t=g.querySelector("table"); if(!t) return;
    const gw=g.getBoundingClientRect().width, tw=t.getBoundingClientRect().width;
    if(tw>0&&gw>0) 比=Math.min(比, gw/tw);
  });
  tabs.forEach(t=>{ t.style.tableLayout=""; t.style.width=""; });
  if(比<1) box.style.fontSize=
    Math.max(3, parseFloat(box.style.fontSize)*比*0.97).toFixed(2)+"px";
  // 縦にあふれる場合もここで詰める(横で決めた字が高さに入らないことがある)。
  for(let i=0;i<10&&box.scrollHeight>box.clientHeight+1;i++){
    const f=parseFloat(box.style.fontSize)*0.94;
    if(f<3) break;
    box.style.fontSize=f.toFixed(2)+"px";
  }
  // ★1つのマスからはみ出す文字が残っていないかを見て、無くなるまで詰める(2026-08-28)。
  //   列の割合は決め打ちなので、期間や日によって桁が増えると必ずどこかが溢れる。
  //   溢れた字は隣の組の機種名の上に重なって出る(谷川氏「罫線がガタガタ」の一因)。
  //   字を小さくすると列の幅(器に対する割合)はそのままで文字だけ縮むので収まる。
  const あふれ=()=>[...box.querySelectorAll('.brg td,.brg th')]
      .some(e=>e.scrollWidth>e.clientWidth+1);
  for(let i=0;i<14&&あふれ();i++){
    const f=parseFloat(box.style.fontSize)*0.96;
    if(f<3) break;
    box.style.fontSize=f.toFixed(2)+"px";
  }
}
try{ window.addEventListener("shimaheat-period",()=>{ try{ 機種別まとめを盤面へ(); }catch(e){} }); }catch(e){}
try{ 機種別まとめを盤面へ(); }catch(e){}

/* ==== 狙い方の答え合わせを島図で見る(2026-09-03・谷川さん指示
   「昨日火曜日時点の直近7日で凹んだAT機の80台の絞り込んだ島図と翌日水曜日の島図を
     タップするだけで押してパッと切り替えてみられるようにしたい。…火曜日時点で
     絞り込んでいた台が翌日水曜ででていたか凹んだかを視覚的にみえるようにしたい」) ====
   材料は資料の「狙い方の実績」と同じ nrec.<hash>.json。週ごとに
   [順位, 台番, 機種, 凹み度(その火曜時点の直近7日平均差枚), 当日差枚, 当日G, 出率]
   を持っているので、**画面で選び直さない**(資料・PDF・Discordと同じ台・同じ数字になる)。
   ★先読みなし。台を選ぶ材料はその水曜より前の記録だけ、というのは作る側(build_nerai_rec)で
     担保されている。ここは出来上がった台帳を見せるだけ。 */
const AW_CLS=["aw-off","aw-hit","aw-chg","aw-chg2","aw-p3","aw-p2","aw-p1","aw-z","aw-m1","aw-m2","aw-m3"];
let awOn=false, awWk=-1, awSide="in";
// 色の段。火曜時点(凹み度=直近7日の平均)と水曜の結果(当日差枚)は桁が違うので、
// 段の切れ目を別々に持つ。決め打ちの1つの物差しで両方を塗ると、片方が全部同じ色になる。
const AW_STEP={ in:[3000,1500,0], out:[3000,1000,0] };
function awColor(v,side){
  if(typeof v!=="number") return "aw-z";
  const s=AW_STEP[side]||AW_STEP.out;
  if(v>0) return v>=s[0]?"aw-p3":(v>=s[1]?"aw-p2":"aw-p1");
  if(v<0) return v<=-s[0]?"aw-m3":(v<=-s[1]?"aw-m2":"aw-m1");
  return "aw-z";
}
function awWeeks(){ return (NREC&&NREC.weeks)||[]; }

/* 盤面の機種名ラベルを「その週の姿」に差し替える(2026-09-03・谷川さん指摘
   「7/1にとある禁書2はまだ導入前だけど、答え合わせで切り替えた時に島図が
     その当時の状態になってない」)。
   材料は SHIMA.awlab（台番機種履歴.json から作った、いまと違うセルだけの差分）。
   ★元の中身は覚えておいて、閉じたら必ず戻す。
   ★いちばん古い版より前(valid_from より前=6/17の週)は材料が無いので触らない。
     推測で当時の名前を作らない。 */
const AW_PUT_ORIG=new Map();
// 当時の島図の材料。素で700KBを超えるので外部ファイルにしてある。
// 答え合わせを開いたときだけ落とす（毎日の表示は重くしない）。
let AWLAB=null, awlabReq=null;
function awlabLoad(){
  if(AWLAB) return Promise.resolve(AWLAB);
  if(typeof SHIMA==="undefined"||!SHIMA||!SHIMA.awlabUrl) return Promise.resolve(null);
  if(awlabReq) return awlabReq;
  awlabReq=fetch(asrc(SHIMA.awlabUrl),{cache:"default"})
    .then(r=>r.ok?r.json():null)
    .then(j=>{ AWLAB=j; return j; })
    .catch(()=>{ awlabReq=null; return null; });
  return awlabReq;
}
function awSnapPick(){
  const AL=AWLAB;
  if(!awOn||!AL||!(AL.snaps||[]).length) return null;
  const w=awWeeks()[awWk]; const ymd=(w&&w.ymd)||"";
  if(!ymd||!AL.valid_from||ymd<AL.valid_from) return null;
  let pick=null;
  for(let i=0;i<AL.snaps.length;i++){
    if(AL.snaps[i].date<=ymd) pick=AL.snaps[i]; else break;
  }
  if(!pick) pick=AL.snaps[0];          // いちばん古い版が効く範囲(valid_from 以降)
  return pick||null;
}
/* 盤面をその週の島図そのものにする(2026-09-03夕4)。
   材料は SHIMA.awlab の版ごとの put(差し替えるセル) と hide(当時は無かったセル)。
   ★台番のセルは触らない（答え合わせの色を出すため）。触るのは機種名・柱・通路・線。
   ★元の中身は控えておき、閉じたら必ず戻す。 */
function awBoardRestore(){
  const board=document.getElementById("board"); if(!board) return;
  AW_PUT_ORIG.forEach((v,k)=>{
    const el=board.querySelector('[data-k="'+k+'"]');
    if(!el) return;
    if(!v.exist){ el.remove(); return; }
    el.removeAttribute("data-awput");
    el.className=v.c;
    el.setAttribute("style",v.s);
    el.innerHTML=v.t;
    if(v.l) el.dataset.lbl=v.l; else delete el.dataset.lbl;
    el.style.display=v.disp||"";
  });
  AW_PUT_ORIG.clear();
  board.querySelectorAll("[data-awhide]").forEach(e=>{
    e.style.display=""; e.removeAttribute("data-awhide");
  });
}
function awBoardApply(){
  const board=document.getElementById("board"); if(!board) return;
  awBoardRestore();
  const sn=awSnapPick(); if(!sn) return;
  const firstLine=board.querySelector(".ln");
  (sn.put||[]).forEach(x=>{
    const k=x[0], c=x[1], st=x[2], t=x[3], l=x[4];
    let el=board.querySelector('[data-k="'+k+'"]');
    if(el){
      AW_PUT_ORIG.set(k,{exist:1, c:el.className, s:el.getAttribute("style")||"",
                         t:el.innerHTML, l:el.dataset.lbl||"", disp:el.style.display});
    }else{
      el=document.createElement("div"); el.dataset.k=k;
      AW_PUT_ORIG.set(k,{exist:0});
      // 線は上に、それ以外(機種名・柱など)は線より下に置く=重なりの順を崩さない
      if((" "+c+" ").indexOf(" ln ")>=0) board.appendChild(el);
      else if(firstLine) board.insertBefore(el, firstLine);
    ...
    if(l) el.dataset.lbl=l; else delete el.dataset.lbl;
    el.style.display="";
    el.setAttribute("data-awput","1");
  });
  (sn.hide||[]).forEach(k=>{
    const e=board.querySelector('[data-k="'+k+'"]');
    if(e){ e.style.display="none"; e.setAttribute("data-awhide","1"); }
  });
}
function awLabApply(){ awBoardApply(); }
function awPaint(){
  const cells=document.querySelectorAll("#board .cell[data-dai]");
  if(!awOn){ cells.forEach(el=>el.classList.remove(...AW_CLS)); awLabApply(); return; }
  const w=awWeeks()[awWk];
  if(!w){ cells.forEach(el=>el.classList.remove(...AW_CLS)); awLabApply(); return; }
  const val=new Map(), chg=new Map();
  (w.rows||[]).forEach(r=>{
    val.set(String(r[1]), awSide==="in"?r[3]:r[4]);
    // r[8]=1 は「その水曜の時点でいまと違う機種だった」台。r[7] に当時の名前が
    // 入っていれば名前まで分かる(2回以上入れ替わった台は名前が残っていない)。
    if(r[8]) chg.set(String(r[1]), r[7]||"");
  });
  cells.forEach(el=>{
    el.classList.remove(...AW_CLS);
    const d=el.dataset.dai;
    if(!val.has(d)){ el.classList.add("aw-off"); return; }
    el.classList.add("aw-hit", awColor(val.get(d), awSide));
    if(chg.has(d)) el.classList.add(chg.get(d)?"aw-chg":"aw-chg2");
  });
  awLabApply();
}
// その週の「いまと違う機種だった台」の数(2026-09-03・谷川さん指摘
// 「過去日でそのときどきの機種構成が異なる」)。名前まで分かる台も数える。
function awChgCount(w){
  const rows=(w&&w.rows)||[];
  const c=rows.filter(r=>r[8]).length;
  return {c:c, named:rows.filter(r=>r[8]&&r[7]).length};
}
function awSummary(){
  const box=document.getElementById("awSum"); if(!box) return;
  const w=awWeeks()[awWk];
  if(!w){ box.textContent=""; return; }
  const vs=(w.rows||[]).map(r=>r[4]).filter(v=>typeof v==="number");
  const plus=vs.filter(v=>v>0).length;
  const avg=vs.length?Math.round(vs.reduce((a,b)=>a+b,0)/vs.length):null;
  const k=awChgCount(w);
  const note=k.c?("／この週は " + k.c + "台がいまと違う機種（点線の枠"
                  + (k.named?("・うち" + k.named + "台は当時の名前も出ます"):"")+"）"):"";
  // 島図の機種名そのものを当時に戻せた週かどうか(2026-09-03)
  const sp=awSnapPick();
  const nput=sp?((sp.put||[]).length):0;
  const ln=sp?("／島図もこの週の姿にしています（機種名・区切り線あわせて"
               + nput + "か所）")
            :"／島図はいまの姿のまま（この週より前の島図が残っていません）";
  box.textContent=((awSide==="in")
    ? ("この" + (w.rows||[]).length + "台を火曜の時点で選びました（色が濃いほど凹んでいた台）")
    : ("翌日 " + w.d + " は " + vs.length + "台中 " + plus + "台がプラス／1台平均 "
       + (avg==null?"−":((avg>0?"+":"")+avg.toLocaleString())) + "枚"))+note+ln;
}
// 台番カードの機種名の行に「当時 ○○」を添える(答え合わせを見ている間だけ)。
// ★入替で中身が変わった台は、いまの機種名だけ見ると当時の話と食い違う。
function awCardNote(dai){
  const box=document.getElementById("msub"); if(!box||!awOn) return;
  const w=awWeeks()[awWk]; if(!w) return;
  const r=(w.rows||[]).find(x=>String(x[1])===String(dai));
  if(!r||!r[8]) return;
  box.textContent += r[7] ? ("　※" + w.d + "当時は " + r[7])
                          : ("　※" + w.d + "当時は別の機種（名前は記録に無し）");
}
function awSync(){
  const bar=document.getElementById("awBar"); if(!bar) return;
  const W=awWeeks();
  bar.hidden=!awOn;
  const bt=document.getElementById("awBtn");
  if(bt) bt.classList.toggle("is-on",awOn);
  if(!awOn) return;
  const w=W[awWk];
  const dt=document.getElementById("awDate");
  if(dt) dt.textContent=w?(w.d+"("+(w.w||"水")+")"):"—";
  const pv=document.getElementById("awPrev"), nx=document.getElementById("awNext");
  if(pv) pv.disabled=(awWk<=0);
  if(nx) nx.disabled=(awWk>=W.length-1);
  const bi=document.getElementById("awIn"), bo=document.getElementById("awOut");
  if(bi) bi.classList.toggle("is-on",awSide==="in");
  if(bo) bo.classList.toggle("is-on",awSide==="out");
  awSummary();
}
function awApply(){ awSync(); awPaint(); }
function awOpen(){
  Promise.all([nrecLoad(), awlabLoad()]).then(([j])=>{
    if(!j||!(j.weeks||[]).length){ showToast("答え合わせの材料がまだありません",2600); return; }
    awOn=true;
    if(awWk<0||awWk>=j.weeks.length) awWk=j.weeks.length-1;   // 既定はいちばん新しい水曜
    awSide="in";
    awApply();
    // 前に動かした場所へ戻す。帯を出してから測るので awApply の後で呼ぶ。
    try{ awPosApply(); }catch(e){}
  }).catch(()=>showToast("答え合わせの材料を読み込めませんでした",2600));
}
function awClose(){ awOn=false; awApply(); }
(function(){
  const bt=document.getElementById("awBtn"); if(!bt) return;
  bt.addEventListener("click",()=>{ awOn?awClose():awOpen(); });
  const on=(id,fn)=>{ const el=document.getElementById(id); if(el) el.addEventListener("click",fn); };
  on("awX",awClose);
  on("awPrev",()=>{ if(awWk>0){ awWk--; awApply(); } });
  on("awNext",()=>{ if(awWk<awWeeks().length-1){ awWk++; awApply(); } });
  on("awIn",()=>{ awSide="in"; awApply(); });
  on("awOut",()=>{ awSide="out"; awApply(); });
  // 期間チップを押すと盤面が貼り替わる(paintCell が class を作り直す)ので、塗り直す。
  try{ window.addEventListener("shimaheat-period",()=>{ if(awOn) awPaint(); }); }catch(e){}
})();

/* ==== 答え合わせの帯を長押しで好きな場所へ動かす(2026-09-03・谷川さん指示
   「答え合わせ時に出てくるカードを押し続けるとカードを任意の位置に移動できるように」) ====
   帯は島図の上に浮いているので、見たい島が隠れることがある。長押しで掴んで逃がす。
   ★短く押したときはボタンの操作のまま。長押し(0.32秒)＋動かしたときだけ移動にする。
   ★置いた場所は覚える(次に開いたときも同じ場所)。画面の外へ出さないよう毎回はめ直す。 */
const AW_POS_KEY="shimaheat-awpos";
function awPosPut(x,y,save){
  const bar=document.getElementById("awBar"); if(!bar||bar.hidden) return;
  const r=bar.getBoundingClientRect();
  const W=window.innerWidth, H=window.innerHeight;
  x=Math.max(4,Math.min(x, W-r.width-4));
  y=Math.max(4,Math.min(y, H-r.height-4));
  bar.style.left=x.toFixed(0)+"px"; bar.style.top=y.toFixed(0)+"px";
  bar.style.transform="none";
  if(save){ try{ localStorage.setItem(AW_POS_KEY, JSON.stringify({x:x,y:y})); }catch(e){} }
}
// 開いたときに覚えている場所へ戻す。画面の向きや端末が変わっても外へ出ないようはめ直す。
function awPosApply(){
  const bar=document.getElementById("awBar"); if(!bar||bar.hidden) return;
  let p=null;
  try{ p=JSON.parse(localStorage.getItem(AW_POS_KEY)||"null"); }catch(e){}
  if(!p||typeof p.x!=="number"||typeof p.y!=="number") return;   // 未設定なら既定の位置
  awPosPut(p.x,p.y,false);
}
(function(){
  const bar=document.getElementById("awBar"); if(!bar) return;
  let hold=null, sx=0, sy=0, ox=0, oy=0, on=false, moved=false, last=0;
  const start=(x,y)=>{
    const r=bar.getBoundingClientRect();
    sx=x; sy=y; ox=r.left; oy=r.top; moved=false;
    hold=setTimeout(()=>{ on=true; bar.classList.add("aw-drag"); },320);
  };
  const move=(x,y)=>{
    if(Math.abs(x-sx)>8||Math.abs(y-sy)>8){
      moved=true;
      if(!on&&hold){ clearTimeout(hold); hold=null; }   // 長押し前に動いたら移動にしない
    }
    if(!on) return;
    awPosPut(ox+(x-sx), oy+(y-sy), false);
  };
  const end=()=>{
    if(hold){ clearTimeout(hold); hold=null; }
    if(on){
      on=false; bar.classList.remove("aw-drag");
      const r=bar.getBoundingClientRect();
      awPosPut(r.left,r.top,true);
      last=Date.now();               // 直後のクリックはボタンに渡さない
    }
  };
  ...
  bar.addEventListener("touchmove",e=>{
    ...
    if(before||on) e.preventDefault();     // 掴んでいる間は画面を動かさない
  },{passive:false});
  ...
  window.addEventListener("mousemove",e=>{ if(hold||on) move(e.clientX,e.clientY); });
  window.addEventListener("mouseup",()=>{ if(hold||on) end(); });
  // 動かした直後のクリックでボタンが誤って効かないようにする
  bar.addEventListener("click",e=>{
    if(Date.now()-last<400){ e.preventDefault(); e.stopPropagation(); }
  },true);
})();
