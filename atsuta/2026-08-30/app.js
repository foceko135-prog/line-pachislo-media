...
function 機種別まとめを盤面へ(){
  ...
  const iPlus=c.indexOf("プラス"), iWin=c.indexOf("勝率");
  ...
  // 画像と同じ並び=台平均の高い順。資料の表は出率順なので、ここだけ並べ替える。
  const rows=(R.rows||[]).slice().sort((a,b)=>
    num((b.v||[])[iAvg])-num((a.v||[])[iAvg]));
  ...
  for(let g=0;g<列;g++){
    ...
    html+='<div class="brg"><table>'
      +'<colgroup>'+[24,8,10,7,10,12,10,19].map(w=>'<col style="width:'+w+'%">').join('')
      +'</colgroup><thead><tr>'
      +'<th class="bn">機種</th><th>台</th><th>プラス</th><th>勝率</th>'
      +'<th>平均G</th><th>出率</th><th>台平均</th><th>総差枚</th></tr></thead><tbody>';
    for(let i=g*行数;i<Math.min((g+1)*行数,rows.length);i++){
      ...
      html+='<tr>'+cell(v[0],"bn")+cell(v[iCnt])
        +cell(iPlus>=0?v[iPlus]:"−")+cell(iWin>=0?v[iWin]:"−")
        +cell(v[iG])
  ...
  if(!box){ box=document.createElement("div"); box.id="boardRates"; bd.appendChild(box); }
