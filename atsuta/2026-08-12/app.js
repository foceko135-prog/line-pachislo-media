...
function renderDay(dai){
  ...
  const H=window.HIRU;
  const hiruOn=!!(H&&H.on&&H.data&&H.data.s&&H.data.s[String(dai)]);
  const m=DATA.machines[dai];
  const s=hiruOn?H.data.s[String(dai)]:((DAYG.s||{})[dai]);
  if(!m) return false;
  ...
  let real=null;
  if(hiruOn) real=(H.data.v||{})[String(dai)];
  ...
  let ser, fixed=false;
  if(noSer){
    if(real==null) return false;         // 当日の数字そのものが無ければ開かない
    ser=[0,real];
  }else{
  ...
  document.getElementById("mtitle").textContent="台"+dai+" "
    +(hiruOn?((H.data.date?hiruDayLabel(H.data.date):"今日")+"午前中 "
  ...
  document.querySelectorAll(".mchip").forEach(b=>{ if(b.id!=="mhiruBtn")
    b.classList.toggle("is-on",!hiruOn&&b.dataset.w==="1"); });
  const lo=Math.min(...ser), hi=Math.max(...ser);
  // 軌跡が無い台は最高・最低に意味が無いので、当日差枚だけを書く(2026-08-11)
  document.getElementById("mcap").textContent=noSer
  ...
  let g=null;
  if(hiruOn){ const kk=(H.data.k||{})[String(dai)]; g=kk&&typeof kk.g==="number"?kk.g:null; }
  ...
  document.getElementById("mbody").innerHTML=
    `<tr><td>${hiruOn?"午前中":wdHtml(dlab)}</td><td class="${cls(v)}">${fmt(v)}</td>`
    +(hiruOn?"<td>−</td>":`<td class="${cls(dacc)}">${fmt(dacc)}</td>`)
...
document.querySelectorAll(".mchip").forEach(b=>{
  b.addEventListener("click",e=>{
    ...
    if(hiruOn&&w!==1&&b.classList.contains("is-on")){
...
function fitTabbar(){
  ...
  for(const id of ["zoomOut","mvClose","ftClose"]){
    ...
    if((id==="mvClose"||id==="ftClose")&&!el.classList.contains("show")){ continue; }
...
function buildDocs(){
  ...
  if(TR.raiten||TR.torizai||TR.keihin||TR.osusume){
    ...
    if(TR.engsha&&TR.engsha.length){
      const rows=TR.engsha.map(e=>{
        const dl=num(e.delta);
        return '<tr><td class="tz-en">'+esc(e.name)+'<span>'+esc(e.cat||"")+'</span></td>'
          +'<td class="tz-c">'+esc(e.n||"")+'</td>'
          +'<td class="tz-c">'+esc(e.maker||"")+'</td>'
          +'<td class="tz-c '+sgn(dl)+'">'+esc(e.delta||"")+'</td>'
          +'<td class="tz-c">'+esc(e.hit||"")+'</td></tr>';
      }).join("");
      html+=sec("engsha","演者・企画ごとの実績",
        '<table class="dengsha"><thead><tr><th>名前</th><th>回数</th><th>推し系</th>'
        +'<th>フロア比</th><th>的中率</th></tr></thead><tbody>'+rows+'</tbody></table>',
        false,{pre:["フロア比＝その演者の推しメーカー系が来店日に店平均をどれだけ"
                    +"上回ったかの平均","回数の少ない人は目安（サンプル不足）"]});
    }
    // ---- 取材 ----
...
function filterStats(dai,per){
  ...
  const w=workStats(dai,per);
  return {name:m.n, sum:sumForDai(dai,per), rate:rateForDai(dai,per),
...
// **BB・RBの回数は日々のデータに無い**(島図のデータは1日ぶんが[差枚, G数]の2つだけで、
// 回数を持っているのは昼スナップ(hiru.json)だけ)。そのため設定推定・ブドウの逆算は
// 午前中の表にしか出せない。ここでは出せるものを、午前中と同じ「台 / 機種全台」の
// 並びで見せる。畳んだ状態で描くので、開くまで縦に伸びない。
...
let detOpen=false, detDai=null;
function paintDetail(dai){
  const el=document.getElementById("mhiru"); if(!el) return;
  if(String(dai)!==String(detDai)){ detOpen=false; detDai=String(dai); }
  const m=DATA.machines[dai];
  ...
  el.innerHTML='<details class="hdet mdet"><summary>詳細（'+esc(perName(per))+"）</summary>"
    ...
    +"BB・RBの回数は日毎のデータに無いので、設定の推定は「今日の午前中」でだけ出ます。"
    +"</div></details>";
  el.hidden=false;
  ...
  if(d){
    d.open=detOpen;
    d.addEventListener("toggle",()=>{ detOpen=d.open; });
  }
}
...
const FNUM=["fVmin","fVmax","fRmin","fRmax","fNmin","fNmax","fGmin","fGmax",
            "fAmin","fAmax","fPmin","fPmax","fDmin","fDmax"];
...
["fSue","fPos","fDow","fToku"].forEach(id=>{
...
// 期間の既定値=直近7日(いちばんよく使う)。**未選択の状態は作らない**
// (期間が無いまま数字だけ入れて「なぜか0台」になるのを防ぐ)。
const PER_DEF={fPer:"last7"};
...
document.getElementById("fApply").addEventListener("click",()=>{
  ...
  document.getElementById("fCount").textContent = noDay
    ...
    : (active ? ("該当: "+hitN+"台 / 839台（期間: "+perLabel()+"）")
              : "条件を入力して絞り込むボタンを押してください");
...
function paintFilterList(){
  ...
  document.querySelectorAll(".tap.filt-hit").forEach(el=>{
    ...
    (g[nm]=g[nm]||[]).push({dai:dai, p:m.p||"",
                            v:sumForDai(dai,per), r:rateForDai(dai,per),
                            a:w.avg, pl:w.plus});
  });
...
document.getElementById("fClear").addEventListener("click",()=>{
  ...
  document.querySelectorAll("#fSue .ch,#fPos .ch,#fDow .ch,#fToku .ch")
...
function paintHiru(dai){
  ...
  const nonNum=(st&&st.labels)
    ...
    : [];
  h+='<div class="est">推定設定＝いちばん近い設定（目安。段の間はその割合で小数）'
...
function fitHiru(){
  const tb=document.querySelector("#mhiru .htbl");
...
(()=>{
  ...
  const off=()=>{
    ...
    window.HIRU={on:false,data:data};
    if(typeof curDai!=="undefined"&&curDai) paintHiru(curDai);
    titleBack(); cellsBack();
